...
function parseVisiblePacket(): VisiblePacket {
  ...
  if (!Number.isInteger(packet.timeBudgetMs) || packet.timeBudgetMs < 1000 || packet.timeBudgetMs > 3_600_000) throw new Error('TRUSTED_CANDIDATE_VISIBLE_PACKET_INVALID');
  ...
  if (!Array.isArray(packet.protectedPaths) || !packet.protectedPaths.every(safePath)) throw new Error('TRUSTED_CANDIDATE_VISIBLE_PACKET_INVALID');
  if (packet.requiredChangedPaths.some(item => packet.protectedPaths.includes(item))) throw new Error('TRUSTED_CANDIDATE_VISIBLE_PACKET_INVALID');
  if (typeof packet.recoveryRequired !== 'boolean') throw new Error('TRUSTED_CANDIDATE_VISIBLE_PACKET_INVALID');
...
async function runCheck(root: string, kind: CheckKind) {
  return new Promise<{ kind: CheckKind; ok: boolean; exitCode: number | null; timedOut: boolean; diagnostic: string }>((resolve, reject) => {
    const child = spawn('/bin/sh', ['-eu', '-c', CHECK_COMMANDS[kind]], {
      cwd: root,
      env: {
        HOME: '/tmp',
    ...
    child.once('close', code => {
      clearTimeout(timer);
      resolve({ kind, ok: !timedOut && code === 0, exitCode: code, timedOut, diagnostic: output });
    });
  });
}
...
async function main(): Promise<void> {
  ...
  const session = await candidateModule.runCodingAgentV14({
    ...
    verify: async () => {
      const rows = [];
      for (const kind of CHECKS) rows.push(await runCheck(root, kind));
      return rows;
    },
    plannerOptions: {
