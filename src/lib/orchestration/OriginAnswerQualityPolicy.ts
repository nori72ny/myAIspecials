...
function buildOriginAnswerQualityInstruction(policy: OriginAnswerQualityPolicy): string {
  ...
  const executiveInstruction = policy.executiveReasoningRequired ? [
    ...
    "- Use task-specific headings only when they improve navigation. Put the conclusion first; integrate intent, immediate actions, longer-term implications, and reasons naturally without repeating them in a fixed template.",
  ...
  return [
    ...
    "- For coding requests, provide complete usable changes with file paths and integration context, meaningful edge cases, and verification steps. Separate tests actually run from tests merely suggested. Never substitute TODO stubs or pseudocode for requested working code unless a concrete limitation prevents completion.",
    "- For plans and proposals, explain the recommended approach, why it fits the supplied situation, dependencies, trade-offs, and concrete next steps. Label illustrative examples and assumptions. Do not pad the answer to an arbitrary word count.",
    "- Respect an explicit request for brevity or depth. For complex requests with no length preference, prioritize a complete, professionally reasoned answer over a short summary; use paragraphs, lists, tables, and fenced code where each helps the user act.",
