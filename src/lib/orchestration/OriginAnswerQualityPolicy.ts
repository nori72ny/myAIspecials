...
const EXECUTIVE_TASKS = new Set<AITaskType>(["decision", "strategy", "research", "current-information"]);
...
export function resolveOriginAnswerQualityPolicy(input: OriginAnswerQualityPolicyInput): OriginAnswerQualityPolicy {
  ...
  const executiveReasoningRequired = EXECUTIVE_TASKS.has(input.taskType) || input.intent.interactionMode === "decision" || decisionRequested;
