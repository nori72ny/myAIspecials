...
export const ORIGIN_CODING_OPENROUTER_FREE_MODEL =
  "nex-agi/nex-n2.5-mini:free" as const;
...
const ORIGIN_OPENROUTER_CODING_FREE_MODEL_SOURCE =
  "https://openrouter.ai/nex-agi/nex-n2.5-mini:free" as const;
...
export const DEFAULT_ORIGIN_FREE_MODEL_CATALOG: readonly OriginFreeModelEvidence[] = [
  {
    ...
    sourceDescription: "OpenRouter official model and provider pages were rechecked on 2026-09-15 JST: the fixed :free model remains listed at zero prompt/completion price. Runtime separately requires ZDR/data-collection-deny routing, max-price zero, exact served-model identity, and zero reported usage cost; any unverifiable or non-zero condition fails closed before an answer is returned.",
  },
  {
    providerId: "openrouter-free",
    providerLabel: "ORIGIN Coding 無料AI",
    modelId: ORIGIN_CODING_OPENROUTER_FREE_MODEL,
    verifiedAt: "2026-09-15T11:10:00.000Z",
    reviewAfter: "2026-09-22T11:10:00.000Z",
    sourceUrl: ORIGIN_OPENROUTER_CODING_FREE_MODEL_SOURCE,
    sourceDescription: "OpenRouter official model page was rechecked on 2026-09-15 JST: Nex-N2.5-Mini is listed as free with zero prompt/completion pricing, 262K context, and support for tools/tool_choice and structured outputs. ORIGIN still enforces ZDR, data_collection=deny, max_price zero, and zero reported usage cost at runtime.",
  },
] as const;
...
export function selectCurrentOriginFreeModel(
  ...
  const current = catalog.find((entry) => {
    const verifiedAt = parseTimestamp(entry.verifiedAt); const reviewAfter = parseTimestamp(entry.reviewAfter);
    return entry.modelId === preferredModel && verifiedAt !== null && reviewAfter !== null && nowMs >= verifiedAt && nowMs <= reviewAfter;
  });
