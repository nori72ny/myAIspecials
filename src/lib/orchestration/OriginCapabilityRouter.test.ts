...
import { capabilityExecutionOrder, selectOriginCapability } from "./OriginCapabilityRouter.js";
...
describe("Origin v2 capability routing", () => {
  ...
  it("reports medium confidence when multiple capabilities tie", () => {
    expect(selectOriginCapability("コードを分析してレビューして")).toMatchObject({ capability: "coding", reason: "keyword", confidence: "medium" });
  });

  it("honors explicit capability over keyword inference", () => {
    expect(selectOriginCapability("最新のコードを調査して", "analysis")).toMatchObject({ capability: "analysis", reason: "explicit", confidence: "high" });
  });
  ...
  it("is deterministic and keeps every capability in the bounded order", () => {
    ...
    expect(capabilityExecutionOrder(decision)).toEqual(["answer", "research", "coding", "writing", "analysis"]);
  });
});
