...
/** Legacy provider identifiers remain exported for compatibility, but are no longer execution-eligible. */
...
export const ORIGIN_ZERO_COST_PROVIDER_IDS: readonly OriginExecutionProviderId[] = [ORIGIN_OPENROUTER_FREE_PROVIDER_ID];
...
export interface OriginProviderDataPolicy { allowProviderFallbacks: false; dataCollection: "deny"; requireZeroDataRetention: false; }
...
function chooseProvider(_taskType: AITaskType, availability: OriginExecutionAvailability): OriginExecutionProviderId {
  return availability.openRouterConfigured ? ORIGIN_OPENROUTER_FREE_PROVIDER_ID : ORIGIN_OPENROUTER_FREE_PROVIDER_ID;
}
...
function resolveProviderEvidence(providerId: OriginExecutionProviderId, planningOptions: OriginExecutionPlanningOptions, nowMs: number): OriginProviderFreeEvidence | OriginExecutionPlanResult {
  if (providerId !== ORIGIN_OPENROUTER_FREE_PROVIDER_ID) return { ok: false, code: "FREE_MODEL_CATALOG_INVALID", message: "許可されていないProviderです。OpenRouterの無料モデルのみ実行可能です。" };
