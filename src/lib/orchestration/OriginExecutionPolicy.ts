...
function chooseOpenRouterModel(request: OriginExecutionRequest): OriginFreeModelId {
  // Only the trusted V1.4 Coding path supplies an explicit implementation task type.
  // Chat/UI requests are classified locally and remain on the globally fixed model,
  // preserving the production response-identity contract in App.tsx.
  return request.taskType === "implementation" && request.requiresCodeChanges === true
...
export function buildOriginExecutionPlan(request: OriginExecutionRequest, availability: OriginExecutionAvailability, policyInput?: Partial<OriginExecutionPolicy>, planningOptions: OriginExecutionPlanningOptions = {}): OriginExecutionPlanResult {
  ...
  const modelId = chooseOpenRouterModel(request);
  ...
  const codingProfile = modelId === ORIGIN_OPENROUTER_CODING_FREE_MODEL;
  return { ok: true, plan: { providerId, providerLabel: codingProfile ? "ORIGIN Coding 無料AI" : "ORIGIN 無料AI", modelId, taskType, capabilityDecision, freeOnly: true, estimatedCostUsd: 0, timeoutMs: policy.timeoutMs, requiresOwnerApproval: false, reason: codingProfile ? `明示的なV1.4 Coding implementation requestとして、用途別に検証済みのOpenRouter無料Codingモデルのみを選択します。Provider自身の無料利用証拠が期限内である場合のみ実行します。` : `依頼を capability「${capabilityDecision.capability}」/ task「${taskType}」としてローカル分類し、検証済みのOpenRouter無料モデルのみを選択します。Provider自身の無料利用証拠が期限内である場合のみ実行します。`, providerDataPolicy, modelEvidence } };
}
