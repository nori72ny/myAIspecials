...
function validate(record: Omit<McpOAuthPending, 'verifierCiphertext' | 'grantId'>): void {
...
const columns = `owner_id AS "ownerId", server_id AS "serverId", state_hash AS "stateHash", session_hash AS "sessionHash",
  config_hash AS "configHash", verifier_ciphertext AS "verifierCiphertext", grant_id AS "grantId"`;
...
export class PostgresMcpOAuthPendingStore implements McpOAuthPendingStore {
  ...
  async put(record: McpOAuthPending): Promise<void> {
    validate(record);
    if (!/^[a-f0-9-]{36}$/.test(record.grantId)) throw new Error('MCP_OAUTH_RECORD_INVALID');
    ...
    try {
      ...
      const result = await client.query(`insert into public.origin_mcp_oauth_pending
        (owner_id, server_id, state_hash, session_hash, config_hash, verifier_ciphertext, grant_id, expires_at)
        select $1, $2, $3, $4, $5, $6, $7::uuid, clock_timestamp() + interval '5 minutes'
        ...
        on conflict (owner_id, server_id) do update set state_hash = excluded.state_hash,
          ...
          verifier_ciphertext = excluded.verifier_ciphertext, grant_id = excluded.grant_id, expires_at = excluded.expires_at
        returning state_hash`, [record.ownerId, record.serverId, record.stateHash, record.sessionHash, record.configHash, record.verifierCiphertext, record.grantId]);
  ...
  async consume(binding: Omit<McpOAuthPending, 'verifierCiphertext' | 'grantId'>): Promise<McpOAuthPending | undefined> {
    validate(binding);
    try {
