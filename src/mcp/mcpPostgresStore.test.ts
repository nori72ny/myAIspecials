...
import { sealMcpCredential, type McpConnectionRecord } from './mcpConnections.js';
...
const encryptedFixture = sealMcpCredential('fixture-token', binding, Buffer.alloc(32, 7));
const record = (overrides: Partial<McpConnectionRecord> = {}): McpConnectionRecord => ({
  ...binding,
  credential: encryptedFixture,
  version: 1,
  status: 'registered',
  checkedAt: null,
  ...overrides,
...
const row = (value = record()) => ({
  ...
  credential_ciphertext: value.credential,
...
describe('PostgresMcpConnectionStore', () => {
  ...
  it('locks before taking a fresh capacity snapshot and commits the insertion', async () => {
    ...
    expect(call.values).toEqual([record().id, record().ownerId, 'docs', record().endpoint, record().credential, 1, 'registered', null, 20]);
  });
  ...
  it('requires owner, immutable connection identity and exact prior version for replacement', async () => {
    ...
    expect(database.calls[0].text).toContain('and endpoint = $8');
    expect(database.calls[0].text).toContain('and version = $9');
