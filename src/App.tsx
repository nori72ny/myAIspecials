...
export const App: React.FC<OriginPersonalAppProps> = ({ onOpenSettings, onOpenResearch, onOpenCoding, onOpenCreative, onOpenDetails, messages: controlledMessages, sessions = [], artifacts: controlledArtifacts, onArchiveSession, onRestoreSession, onMessagesChange, onArtifactsChange, resetSignal = 0, language = 'ja', designTheme = 'minimal', embedded = false }) => {
  ...
  <div role="menu" aria-label={language === 'ja' ? '追加機能' : 'Additional tools'} className="origin-add-menu__panel origin-surface absolute bottom-14 right-0 z-50 grid min-w-52 gap-1 rounded-2xl border border-[var(--border-default)] p-2 shadow-2xl">
    <button type="button" role="menuitem" onClick={(event) => { closeComposerMenu(event.currentTarget); fileInputRef.current?.click(); }} className="origin-secondary-button min-h-11 rounded-[10px] px-3 text-left text-sm">{language === 'ja' ? 'ファイルを添付' : 'Attach file'}</button>
    {onOpenResearch && <button type="button" role="menuitem" onClick={(event) => { closeComposerMenu(event.currentTarget); onOpenResearch(); }} className="origin-secondary-button min-h-11 rounded-[10px] px-3 text-left text-sm">{language === 'ja' ? '調べる' : 'Research'}</button>}
    {onOpenCoding && <button type="button" role="menuitem" onClick={(event) => { closeComposerMenu(event.currentTarget); onOpenCoding(); }} className="origin-secondary-button min-h-11 rounded-[10px] px-3 text-left text-sm">{language === 'ja' ? 'コード' : 'Code'}</button>}
    {onOpenCreative && <button type="button" role="menuitem" onClick={(event) => { closeComposerMenu(event.currentTarget); onOpenCreative(); }} className="origin-secondary-button min-h-11 rounded-[10px] px-3 text-left text-sm">{language === 'ja' ? '作る' : 'Create'}</button>}
    {onOpenDetails && <button type="button" role="menuitem" onClick={(event) => { closeComposerMenu(event.currentTarget); onOpenDetails(); }} className="origin-secondary-button min-h-11 rounded-[10px] px-3 text-left text-sm">{language === 'ja' ? '詳細' : 'Details'}</button>}
  </div>
