...
export const ArtifactWorkspace: React.FC<{ artifact: ArtifactBlock | null; artifacts?: readonly ArtifactBlock[]; isOpen: boolean; language: OriginLanguage; onClose: () => void; onOpenSettings?: () => void; onArtifactRevision?: (artifact: ArtifactBlock) => void; isStreaming?: boolean; onSteer?: (direction: string) => void; designTheme?: OriginDesignTheme }> = ({ artifact, artifacts = [], isOpen, language, onClose, onOpenSettings, onArtifactRevision, isStreaming = false, onSteer, designTheme = 'minimal' }) => {
  ...
  const [activeTab, setActiveTab] = useState<'preview' | 'code'>(() => artifact && (artifact.type === 'html' || artifact.language === 'html' || artifact.language === 'svg') ? 'preview' : 'code');
  ...
  useEffect(() => { cleanLoadConfirmed.current = false; setActiveTab(artifact && (artifact.type === 'html' || artifact.language === 'html' || artifact.language === 'svg') ? 'preview' : 'code'); setCopied(false); setShared(false); setIsPresentation(false); setPresentationSlideIndex(0); setPreviewViewport('fluid'); setIsDirectEditing(false); setSandboxError(null); setLastKnownGood(null); setIsExportMenuOpen(false); setIsDetailsMenuOpen(false); }, [artifact?.id]);
