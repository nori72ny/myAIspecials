...
export type OriginPersonalAppProps = { onOpenSettings?: () => void; messages?: ConversationMessage[]; sessions?: readonly ConversationSession[]; onArchiveSession?: (messages: readonly ConversationMessage[]) => void; onRestoreSession?: (session: ConversationSession) => void; onMessagesChange?: (messages: ConversationMessage[]) => void; resetSignal?: number; language?: OriginLanguage };
export const App: React.FC<OriginPersonalAppProps> = ({ onOpenSettings, messages: controlledMessages, sessions = [], onArchiveSession, onRestoreSession, onMessagesChange, resetSignal = 0, language = 'ja' }) => {
  const t = getTranslations(language);
  ...
  const resetConversation = () => { onArchiveSession?.(messagesRef.current); abortRef.current?.abort(); abortRef.current = null; setIsLoading(false); setInputText(''); setAttachments([]); setAttachmentError(''); setActiveArtifact(null); setArtifacts([]); setIsWorkspaceOpen(false); updateMessages(() => []); };
