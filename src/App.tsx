...
export const ArtifactWorkspace: React.FC<{ artifact: ArtifactBlock | null; isOpen: boolean; language: OriginLanguage; onClose: () => void }> = ({ artifact, isOpen, language, onClose }) => {
  ...
  useEffect(() => { cleanLoadConfirmed.current = false; setActiveTab('code'); setCopied(false); setShared(false); setIsDirectEditing(false); setSandboxError(null); setLastKnownGood(null); }, [artifact?.id]);
  ...
  const sandboxSrcDoc = useMemo(() => {
    ...
    const sanitized = `${cleanLoadBoundary}${workingContent.replace(/<meta[^>]*http-equiv\s*=\s*["']?refresh["']?[^>]*>/gi, '')}`;
