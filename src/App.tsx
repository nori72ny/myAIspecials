...
export const App: React.FC<OriginPersonalAppProps> = ({ onOpenSettings, messages: controlledMessages, sessions = [], artifacts: controlledArtifacts, onArchiveSession, onRestoreSession, onMessagesChange, onArtifactsChange, resetSignal = 0, language = 'ja', designTheme = 'minimal' }) => {
  ...
  return <div className={`origin-app flex w-full overflow-hidden font-sans ${messages.length ? 'h-[100dvh]' : 'h-auto'}`}><main className="relative flex h-full min-w-0 flex-1 flex-col overflow-hidden"><header className="origin-header flex min-h-16 items-center justify-between px-3 backdrop-blur-md sm:px-4">
