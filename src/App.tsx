...
// ==========================================
// 1. ストリーミング成果物パーサー (独立監査 FINAL PASS)
// ==========================================
...
// ==========================================
// 2. セキュア成果物ワークスペース (ArtifactWorkspace)
// ==========================================
...
// ==========================================
// 3. メインアプリケーション (App) - E2Eテスト & 新UI完全共存
// ==========================================
    ...
    <div className="flex h-screen w-screen overflow-hidden bg-slate-950 text-slate-100 font-sans">
      <main className="flex-1 flex flex-col h-full overflow-hidden relative">
        ...
        <header className="h-14 px-4 border-b border-slate-800 flex items-center justify-between bg-slate-900/80 backdrop-blur-md">
          ...
          {messages.length > 0 && (
            <button onClick={() => { setMessages([]); setActiveArtifact(null); setIsWorkspaceOpen(false); }} className="text-xs px-2.5 py-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-300">
              新規対話
            </button>
          )}
        </header>
                ...
                <div className="mt-3 pt-3 border-t border-slate-800/80 flex items-center justify-between">
                  ...
                  <button
                    ...
                    className={`px-4 py-1.5 rounded-xl text-xs font-bold transition-all ${
                      inputText.trim() ? 'bg-cyan-500 text-slate-950 shadow-md' : 'bg-slate-800 text-slate-500'
                    }`}
                  >
                    始める →
                  </button>
                  ...
                  {starterCards.map((c, i) => (
                    ...
                    >
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-[10px] text-cyan-500/80 font-mono">{c.subtitle}</span>
                      </div>
          ...
          <div className="w-full max-w-3xl mx-auto px-4 pb-4">
            <div className="flex items-end gap-2 p-2 rounded-2xl bg-slate-900 border border-slate-800 focus-within:border-cyan-500/50 shadow-lg">
              <textarea
                ...
                rows={1}
                disabled={isLoading}
                className="flex-1 max-h-40 resize-none bg-transparent px-3 py-2 text-sm text-slate-100 placeholder-slate-500 focus:outline-none"
              />
              <button
                onClick={() => handleSend()}
                disabled={!inputText.trim() || isLoading}
...
export default App;        <p className="text-sm text-slate-400 mt-2 text-center max-w-md font-sans">
