...
export const ORIGIN_OFFLINE_MESSAGE = '現在オフラインです（保存済み成果物の閲覧・編集のみ可能です）';
const ORIGIN_OFFLINE_MESSAGE_EN = 'You are currently offline. Saved artifacts remain available for viewing and editing.';
...
export const getOriginSystemPrompt = (language: OriginLanguage): string => language === 'en'
  ...
  : 'あなたは ORIGIN Personal 2.0 です。結論を1文で先に示し、根拠、比較、リスク、次の行動を論理的に整理してください。成果物は ```language:title 形式で、本番利用できる品質に仕上げてください。';
...
const KnowledgeMap: React.FC<{ sessions: readonly ConversationSession[]; artifacts?: readonly ArtifactBlock[]; onRestoreSession?: (session: ConversationSession) => void; onOpenArtifact?: (artifact: ArtifactBlock) => void }> = ({ sessions, artifacts = [], onRestoreSession, onOpenArtifact }) => <HistoryDrawer sessions={sessions} artifacts={artifacts} onRestoreSession={onRestoreSession} onOpenArtifact={onOpenArtifact} />;
...
export const App: React.FC<OriginPersonalAppProps> = ({ onOpenSettings, messages: controlledMessages, sessions = [], artifacts: controlledArtifacts, onArchiveSession, onRestoreSession, onMessagesChange, onArtifactsChange, resetSignal = 0, language = 'ja', designTheme = 'minimal' }) => {
  ...
  const handleSend = async (textToSend?: string, interruptCurrent = false) => {
    ...
    if (isOffline || !navigator.onLine) {
      setAttachmentError(language === 'en' ? ORIGIN_OFFLINE_MESSAGE_EN : ORIGIN_OFFLINE_MESSAGE);
      setIsSafeWaiting(false);
      return;
    }
    setAttachmentError('');
