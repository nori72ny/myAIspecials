...
import OriginRuntimeActivityTimelineV31, { type OriginRuntimeActivityV31 } from './components/personal/OriginRuntimeActivityV31';
...
const ResponseVerificationBadge: React.FC<{ language: OriginLanguage }> = ({ language }) => <details data-testid="response-verification-details" className="origin-verification mt-2 w-full"><summary className="origin-secondary-button flex min-h-11 min-w-11 list-none items-center gap-2 rounded-[10px] px-3 text-[13px] font-semibold"><span aria-hidden="true">✓</span><span>{language === 'en' ? '$0 delivery confirmed' : '$0配信を確認'}</span><span className="origin-muted ml-auto text-[13px] font-normal">{language === 'en' ? 'Scope' : '確認範囲'}</span></summary><ul data-testid="response-verification-log" className="origin-surface-muted m-0 mt-2 grid gap-1 rounded-2xl p-3 text-[13px] leading-6"><li><span className="font-semibold">{language === 'en' ? 'Completion' : '応答完了'}</span> · {language === 'en' ? 'A complete response frame was received.' : '完了した応答フレームを受信しました。'}</li><li><span className="font-semibold">{language === 'en' ? 'Cost route' : '費用経路'}</span> · {language === 'en' ? 'The fixed free-model route reported $0.' : '固定の無料モデル経路が$0と報告したことを確認しました。'}</li><li><span className="font-semibold">{language === 'en' ? 'Content limits' : '内容の限界'}</span> · {language === 'en' ? 'This badge does not mean sources or an independent AI verified the answer.' : '出典確認や別AIによる内容検証を示すものではありません。'}</li></ul></details>;
...
export type OriginPersonalAppProps = { embedded?: boolean; openArtifactId?: string | null; onArtifactOpen?: (artifact: ArtifactBlock | null) => void; composerControls?: React.ReactNode; runtimeActivities?: readonly OriginRuntimeActivityV31[]; onOpenSettings?: () => void; messages?: ConversationMessage[]; sessions?: readonly ConversationSession[]; artifacts?: readonly ArtifactBlock[]; onArchiveSession?: (messages: readonly ConversationMessage[]) => void; onRestoreSession?: (session: ConversationSession) => void; onMessagesChange?: (messages: ConversationMessage[]) => void; onArtifactsChange?: (artifacts: ArtifactBlock[]) => void; resetSignal?: number; language?: OriginLanguage; designTheme?: OriginDesignTheme };
export const App: React.FC<OriginPersonalAppProps> = ({ embedded = false, openArtifactId, onArtifactOpen, composerControls, runtimeActivities = [], onOpenSettings, messages: controlledMessages, sessions = [], artifacts: controlledArtifacts, onArchiveSession, onRestoreSession, onMessagesChange, onArtifactsChange, resetSignal = 0, language = 'ja', designTheme = 'minimal' }) => {
  const t = getTranslations(language);
  const [uncontrolledMessages, setUncontrolledMessages] = useState<ConversationMessage[]>([]);
  const [inputText, setInputText] = useState('');
  const [isLoading, setIsLoading] = useState(false);
  const [isOffline, setIsOffline] = useState(() => typeof navigator !== 'undefined' && !navigator.onLine);
  const [localActiveArtifact, setActiveArtifact] = useState<ArtifactBlock | null>(null);
  ...
  const activeArtifact = embedded ? (artifacts.find(artifact => artifact.id === openArtifactId) ?? null) : localActiveArtifact;
  ...
  const resetConversation = () => { onArchiveSession?.(messagesRef.current); abortRef.current?.abort(); abortRef.current = null; setIsLoading(false); setInputText(''); setAttachments([]); setAttachmentError(''); setIsSafeWaiting(false); setActiveArtifact(null); setIsWorkspaceOpen(false); onArtifactOpen?.(null); updateMessages(() => []); };
