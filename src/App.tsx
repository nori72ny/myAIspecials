...
export const ArtifactWorkspace: React.FC<{ artifact: ArtifactBlock | null; isOpen: boolean; language: OriginLanguage; onClose: () => void }> = ({ artifact, isOpen, language, onClose }) => {
  ...
  const sandboxSrcDoc = useMemo(() => {
    ...
    const sanitized = `${cleanLoadBoundary}${presentationBridge}${presentationStyles}<div data-origin-presentation-content data-origin-presenting="${isPresentation ? 'true' : 'false'}">${markedContent.replace(/<meta[^>]*http-equiv\s*=\s*["']?refresh["']?[^>]*>/gi, '')}</div>`;
    ...
    return `<!doctype html><html><head><meta charset="utf-8"><meta http-equiv="Content-Security-Policy" content="default-src 'none'; script-src 'unsafe-inline'; style-src 'unsafe-inline'; img-src data: blob:; connect-src 'none'; font-src 'none'; media-src 'none'; object-src 'none'; frame-src 'none'; child-src 'none'; worker-src 'none'; manifest-src 'none'; form-action 'none'; base-uri 'none';"><meta name="referrer" content="no-referrer"><style>html,body{min-height:100%;margin:0}body{font-family:system-ui,sans-serif;padding:16px;color:CanvasText;background:transparent}[data-origin-direct-touch="true"]{outline:2px dashed #22d3ee;outline-offset:-4px;cursor:text}</style></head><body>${directTouch}<script>window.addEventListener('click',function(event){var target=event.target;while(target&&target.tagName!=='A')target=target.parentElement;if(target&&target.tagName==='A'){event.preventDefault();event.stopPropagation();}},true);window.open=function(){return null;};</script>${sanitized}</body></html>`;
  ...
  useEffect(() => {
    const onSandboxMessage = (event: MessageEvent) => {
      ...
      const data = event.data as { source?: string; type?: string; message?: string; content?: string; timestamp?: number };
  ...
  return <aside ref={workspaceRef} aria-label={t.workspaceLabel} data-testid="artifact-workspace" className="origin-workspace fixed inset-y-0 right-0 z-50 flex w-full flex-col border-l shadow-2xl sm:w-[560px]">
    <div className="flex min-h-16 flex-wrap items-center justify-between gap-2 border-b border-[var(--border-default)] px-3 py-2 sm:px-4">
      <div className="min-w-0 flex flex-1 items-center gap-2"><span className="origin-badge rounded-md border px-2 py-1 text-xs font-mono font-semibold">{artifact.language}</span><h2 className="truncate text-sm font-semibold">{artifact.title}</h2></div>
      ...
      {isPresentation && <p className="sr-only" role="status">{t.presentationKeyboardHint}</p>}
    </div>
