...
import { OwnerImprovementInbox } from './components/owner/OwnerImprovementInbox';
import { getTranslations, type OriginLanguage } from './i18n';
import { originIndexedDbAdapter } from './lib/local/OriginIndexedDb';
import { addOwnerImprovement } from './lib/local/OwnerImprovementStore';
import { detectExplicitOwnerImprovementIntent } from './lib/owner/OwnerImprovementIntent';
...
const HistoryDrawer: React.FC<{ sessions: readonly ConversationSession[]; artifacts?: readonly ArtifactBlock[]; onRestoreSession?: (session: ConversationSession) => void; onOpenArtifact?: (artifact: ArtifactBlock) => void }> = ({ sessions, artifacts = [], onRestoreSession, onOpenArtifact }) => {
  ...
  const searchableSessions = useMemo(() => {
    ...
    return [...storedSessions.filter((session) => !liveIds.has(session.id)), ...sessions];
  }, [sessions, storedSessions]);
  const searchableArtifacts = useMemo(() => {
    if (storedArtifacts === null) return artifacts;
    const liveIds = new Set(artifacts.map((artifact) => artifact.id));
    return [...storedArtifacts.filter((artifact) => !liveIds.has(artifact.id)), ...artifacts];
...
export const App: React.FC<OriginPersonalAppProps> = ({ onOpenSettings, messages: controlledMessages, sessions = [], artifacts: controlledArtifacts, onArchiveSession, onRestoreSession, onMessagesChange, onArtifactsChange, resetSignal = 0, language = 'ja', designTheme = 'minimal' }) => {
  ...
  const resetConversation = () => { onArchiveSession?.(messagesRef.current); abortRef.current?.abort(); abortRef.current = null; setIsLoading(false); setInputText(''); setAttachments([]); setAttachmentError(''); setImprovementNotice(''); setIsSafeWaiting(false); setActiveArtifact(null); setIsWorkspaceOpen(false); updateMessages(() => []); };
  ...
  const handleSend = async (textToSend?: string, interruptCurrent = false) => {
    ...
    setIsSafeWaiting(false);
    const improvementIntent = detectExplicitOwnerImprovementIntent(text.trim());
    if (improvementIntent.matched) {
      void addOwnerImprovement({ body: text.trim(), category: improvementIntent.category, source: "chat-routing" })
        .then(() => setImprovementNotice(language === 'en'
          ? 'Saved as an Owner improvement request while continuing this chat.'
          : 'Owner改善案件として保存し、このチャットも続けます。'))
        .catch(() => undefined);
    }
  ...
  <div className="flex items-center gap-2">
    <div className="flex shrink-0 items-center gap-1 whitespace-nowrap sm:gap-2"><span className="h-2.5 w-2.5 rounded-full bg-[var(--accent-primary)] shadow-[0_0_10px_var(--accent-glow)]" aria-hidden="true" /><span className="text-base font-extrabold tracking-tight">ORIGIN</span><span className="origin-badge hidden rounded-[10px] px-1.5 py-0.5 text-[13px] font-mono min-[360px]:inline-flex">Personal 2.0</span></div><div className="flex shrink-0 items-center gap-1 whitespace-nowrap sm:gap-2"><KnowledgeMap sessions={sessions} onRestoreSession={onRestoreSession} /><OwnerImprovementInbox language={language} /><button type="button" onClick={onOpenSettings} aria-label={t.openSettings} className="origin-secondary-button inline-flex h-11 min-h-11 w-11 min-w-11 shrink-0 items-center justify-center whitespace-nowrap rounded-[10px] px-0 text-[13px] font-semibold sm:w-auto sm:px-3"><span aria-hidden="true">⚙️</span><span className="hidden sm:ml-1.5 sm:inline">{t.settings}</span></button><button type="button" onClick={resetConversation} aria-label={t.newConversationLabel} className="origin-secondary-button inline-flex h-11 min-h-11 w-11 min-w-11 shrink-0 items-center justify-center whitespace-nowrap rounded-[10px] px-0 text-[13px] font-semibold sm:w-auto sm:px-3"><span aria-hidden="true">＋</span><span className="hidden sm:ml-1.5 sm:inline">{t.newConversation}</span></button></div>
  </div>
