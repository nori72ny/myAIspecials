...
export const App: React.FC<OriginPersonalAppProps> = ({ onOpenSettings, messages: controlledMessages, sessions = [], artifacts: controlledArtifacts, onArchiveSession, onRestoreSession, onMessagesChange, onArtifactsChange, resetSignal = 0, language = 'ja', designTheme = 'minimal', embedded = false }) => {
  const t = getTranslations(language);
  ...
  return <div className={`origin-app flex ${embedded ? "h-full w-full" : "h-[100dvh] w-screen"} overflow-hidden font-sans`}><main className="relative flex h-full min-w-0 flex-1 flex-col overflow-hidden"><header className="origin-header flex min-h-16 items-center justify-between px-3 backdrop-blur-md sm:px-4">
