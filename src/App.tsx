...
export const getOriginSystemPrompt = (language: OriginLanguage): string => language === 'en'
  ...
  : 'あなたは ORIGIN Personal です。結論を1文で先に示し、根拠、比較、リスク、次の行動を論理的に整理してください。簡潔さを情報不足と取り違えず、複雑な依頼では明示された要件をすべて扱い、重要なニュアンスを落とさない専門的で実用的な回答にしてください。確認済みの事実と推論・仮定を区別し、出典や完了実績を創作せず、重要な未確認点は明示してください。最新情報や外部事実が結論に影響する場合は利用可能な根拠に結び付け、確認できない場合は未確認と明記してください。複雑な依頼では短い概要だけで打ち切らず、実行に必要な詳細、条件、検証方法、具体的な進め方まで十分に示し、単純な依頼は簡潔にしてください。作成物は ```language:title 形式で、本番利用できる品質に仕上げてください。作成依頼では、単にコードや雛形を返すのではなく、ユーザーが実際に達成したい目的を会話履歴から引き継ぎ、使える完成形まで設計してください。選択肢によって保存方法・共有範囲・安全性などが大きく変わる場合だけ、短い確認質問を1つに絞ってください。それ以外は妥当な初期値を置いて先に実用的な第1版を作り、後から改善できる形にしてください。既存の表や作成物をWebアプリ化する依頼では、元の項目と意図を保持しながら、登録・編集・削除、自動計算、入力検証、分かりやすい空状態、集計、端末内保存、CSV等の書き出し、スマートフォン対応など、その用途で自然に期待される機能を不足なく検討してください。ユーザーが特定の題材を指定していない限り、ToDo・タスク管理・天気・カウンターなどの汎用デモを勝手に作成物の題材へ選ばず、依頼内容そのものに沿った実用的なものを作ってください。HTMLやWeb UI作成物は、未装飾のブラウザ標準UIをそのまま出さず、モバイル優先で入力欄・ボタンを44px以上、十分な余白、明確な階層、読みやすいタイポグラフィ、レスポンシブなレイアウトを備えた完成品として生成してください。作成後は、含めた機能、使い方、保存範囲や制約、次に拡張できる内容を短く説明してください。';
...
const ResponseVerificationBadge: React.FC<{ language: OriginLanguage }> = ({ language }) => <details data-testid="response-verification-details" className="origin-verification mt-0.5 w-full">
  ...
  <ul data-testid="response-verification-log" className="m-0 mt-1 grid gap-1 border-l border-[var(--border-default)] px-3 py-2 text-[13px] leading-6">
    <li><span className="font-semibold">{language === 'en' ? 'Completion' : '応答完了'}</span> · {language === 'en' ? 'A complete response frame was received.' : '完了した応答フレームを受信しました。'}</li>
    <li><span className="font-semibold">{language === 'en' ? 'Cost route' : '費用経路'}</span> · {language === 'en' ? 'The fixed free-model route reported $0.' : '固定の無料モデル経路が$0と報告したことを確認しました。'}</li>
    <li><span className="font-semibold">{language === 'en' ? 'Content limits' : '内容の限界'}</span> · {language === 'en' ? 'This does not mean sources or an independent AI verified the answer.' : '出典確認や別AIによる内容検証を示すものではありません。'}</li>
  </ul>
</details>;
...
const artifactPrefersPreview = (artifact: ArtifactBlock | null): boolean => Boolean(artifact && (
  artifact.type === 'html'
...
export const ArtifactWorkspace: React.FC<{ artifact: ArtifactBlock | null; artifacts?: readonly ArtifactBlock[]; isOpen: boolean; language: OriginLanguage; onClose: () => void; onOpenSettings?: () => void; onArtifactRevision?: (artifact: ArtifactBlock) => void; isStreaming?: boolean; onSteer?: (direction: string) => void; designTheme?: OriginDesignTheme }> = ({ artifact, artifacts = [], isOpen, language, onClose, onOpenSettings, onArtifactRevision, isStreaming = false, onSteer, designTheme = 'minimal' }) => {
  ...
  const [activeTab, setActiveTab] = useState<'preview' | 'code'>(() => artifactPrefersPreview(artifact) ? 'preview' : 'code');
  ...
  const isSandboxRenderable = Boolean(artifact && (artifact.type === 'html' || artifact.language === 'html' || artifact.language === 'svg'));
  const isMarkdownRenderable = Boolean(artifact && (artifact.type === 'markdown' || artifact.language === 'markdown' || artifact.language === 'md'));
  const isRenderable = isSandboxRenderable || isMarkdownRenderable;
  const sandboxSrcDoc = useMemo(() => {
    if (!artifact || !isSandboxRenderable) return '';
    ...
    return `<!doctype html><html><head><meta charset="utf-8"><meta http-equiv="Content-Security-Policy" content="default-src 'none'; script-src 'unsafe-inline'; style-src 'unsafe-inline'; img-src data: blob:; connect-src 'none'; font-src 'none'; media-src 'none'; object-src 'none'; frame-src 'none'; child-src 'none'; worker-src 'none'; manifest-src 'none'; form-action 'none'; base-uri 'none';"><meta name="referrer" content="no-referrer"><style>html,body{min-height:100%;margin:0}body{font-family:system-ui,-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;padding:clamp(18px,5vw,32px);color:var(--text-primary,CanvasText);background:var(--bg-primary,transparent);transition:background-color 180ms ease,color 180ms ease;line-height:1.55}h1,h2,h3{letter-spacing:-.025em;line-height:1.15}input:not([type='checkbox']):not([type='radio']),textarea,select{box-sizing:border-box;min-height:46px;max-width:100%;border:1px solid color-mix(in srgb,CanvasText 18%,transparent);border-radius:10px;padding:10px 12px;background:Canvas;color:CanvasText;font:inherit;font-size:16px}input:not([type='checkbox']):not([type='radio']){width:min(100%,24rem)}button{min-height:44px;border:1px solid color-mix(in srgb,CanvasText 16%,transparent);border-radius:10px;padding:9px 14px;background:color-mix(in srgb,Canvas 92%,CanvasText 8%);color:CanvasText;font:600 15px/1.2 system-ui,-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;cursor:pointer}button:focus-visible,input:focus-visible,textarea:focus-visible,select:focus-visible{outline:2px solid #6366f1;outline-offset:2px}[data-origin-direct-touch="true"]{outline:2px dashed #22d3ee;outline-offset:-4px;cursor:text}[data-origin-direct-touch="true"] [data-origin-direct-touch-index]{outline:1px dashed color-mix(in srgb,#22d3ee 55%,transparent);outline-offset:3px;border-radius:3px}</style></head><body><script>window.addEventListener('click',function(event){var target=event.target;while(target&&target.tagName!=='A')target=target.parentElement;if(target&&target.tagName==='A'){event.preventDefault();event.stopPropagation();}},true);window.open=function(){return null;};</script>${sanitized}</body></html>`;
  }, [artifact, isDirectEditing, isPresentation, isSandboxRenderable, language, presentationSlideIndex, workingContent]);
  ...
  useEffect(() => {
    if (!isOpen || !isSandboxRenderable || activeTab !== 'preview') return;
  ...
  }, [activeTab, designTheme, isOpen, isSandboxRenderable]);
  useEffect(() => {
  ...
  const codePanel = isDirectEditing
    ? <div className="flex h-full min-h-0 flex-col gap-3"><label htmlFor="artifact-code-editor" className="text-[13px] font-semibold">{language === 'ja' ? '作成物のコードを直接編集' : 'Edit artifact source'}</label><textarea ref={codeEditorRef} id="artifact-code-editor" data-testid="artifact-code-editor" aria-label={language === 'ja' ? '作成物のコードを直接編集' : 'Edit artifact source'} aria-invalid={!syntaxResult.valid} aria-describedby="artifact-code-syntax-status" value={workingContent} onChange={updateCodeEditor} onKeyDown={(event) => { if (event.nativeEvent.isComposing || event.keyCode === 229) return; if ((event.metaKey || event.ctrlKey) && event.key === 'Enter') { event.preventDefault(); commitCodeRevision(); } }} spellCheck={false} className="origin-surface min-h-48 flex-1 resize-none rounded-2xl border border-[var(--border-default)] p-3 font-mono text-[13px] leading-6 focus:outline-none focus:ring-2 focus:ring-[var(--accent-glow)]" /><div id="artifact-code-syntax-status" data-testid="artifact-code-syntax-status" role={syntaxResult.valid ? 'status' : 'alert'} className={`text-[13px] ${syntaxResult.valid ? 'text-[var(--success)]' : 'text-[var(--danger)]'}`}>{syntaxResult.valid ? language === 'ja' ? '✓ 構文を確認済み。閉じタグは自動補完されます。' : '✓ Syntax verified. Closing tags are completed automatically.' : language === 'ja' ? `${syntaxResult.issue.line}行 ${syntaxResult.issue.column}列: ${syntaxResult.issue.message}` : `Line ${syntaxResult.issue.line}, column ${syntaxResult.issue.column}: ${syntaxResult.issue.message}`}</div><button type="button" data-testid="artifact-code-apply" disabled={!syntaxResult.valid} onClick={commitCodeRevision} className="origin-primary-button min-h-11 min-w-11 rounded-[10px] px-4 text-[13px] font-semibold disabled:cursor-not-allowed disabled:opacity-50">{language === 'ja' ? '確認して編集を完了' : 'Validate and finish editing'}</button></div>
    : <pre className="m-0 whitespace-pre-wrap break-all font-mono text-[13px] leading-6">{workingContent}</pre>;
  const previewFrame = activeTab === 'preview' && isSandboxRenderable
    ? <div data-testid="responsive-preview-stage" className="flex h-full min-w-full items-start justify-center overflow-auto"><iframe ref={previewRef} title={t.previewTitle} aria-label={t.previewTitle} key={sandboxSrcDoc} src="/origin-artifact-sandbox.html" data-origin-srcdoc={sandboxSrcDoc} sandbox={ORIGIN_ARTIFACT_SANDBOX_PERMISSIONS} referrerPolicy="no-referrer" onLoad={(event) => { event.currentTarget.setAttribute('data-origin-loaded', 'true'); event.currentTarget.contentWindow?.postMessage({ source: 'ORIGIN_SANDBOX_INIT', html: sandboxSrcDoc }, '*'); postArtifactTheme(); if (isPresentation) postPresentationCommand('presentation-start'); }} style={{ width: previewWidth }} className="origin-surface h-full shrink-0 rounded-2xl transition-[width] duration-200 ease-out motion-reduce:transition-none" /></div>
    : activeTab === 'preview' && isMarkdownRenderable
      ? <div data-testid="artifact-markdown-preview" className="origin-surface h-full overflow-auto rounded-2xl p-4 sm:p-6"><OriginAnswerMarkdown content={workingContent} language={language} /></div>
      : codePanel;
  ...
  return <aside ref={workspaceRef} aria-label={t.workspaceLabel} data-testid="artifact-workspace" className="origin-workspace fixed inset-y-0 right-0 z-50 flex w-full flex-col border-l shadow-2xl sm:w-[72vw] sm:max-w-[760px]">
    <div className="artifact-workspace__header min-h-16 border-b border-[var(--border-default)] px-3 py-2 sm:px-4">
      <div className="artifact-workspace__title min-w-0 flex items-center gap-2"><span className="origin-badge rounded-[10px] px-2 py-1 text-[13px] font-mono font-semibold">{artifact.language}</span><h2 className="truncate text-base font-semibold">{artifact.title}</h2>{priorRevision && <span className="origin-muted shrink-0 text-[12px]">更新あり</span>}</div>
      <div className="artifact-workspace__controls">
        <div className="artifact-workspace__view-controls">{isRenderable && <div role="group" aria-label={t.displayMode} className="flex items-center gap-1"><button type="button" aria-label={t.showPreview} aria-pressed={activeTab === 'preview'} onClick={() => { if (!isDirectEditing || syntaxResult.valid) setActiveTab('preview'); }} className={`min-h-11 rounded-lg px-3 text-[13px] font-semibold ${activeTab === 'preview' ? 'origin-primary-button' : 'origin-secondary-button'}`}>{language === 'ja' ? '表示' : 'Preview'}</button><button type="button" aria-label={t.showCode} aria-pressed={activeTab === 'code'} onClick={() => setActiveTab('code')} className={`min-h-11 rounded-lg px-3 text-[13px] font-medium ${activeTab === 'code' ? 'origin-primary-button' : 'origin-secondary-button'}`}>{language === 'ja' ? 'コード' : 'Code'}</button></div>}</div>
        <div className="artifact-workspace__actions"><div data-testid="artifact-action-bar" role="group" aria-label="作成物の主要操作" className="flex items-center gap-1"><button type="button" data-testid="artifact-action-edit" aria-label={t.editArtifact} aria-pressed={isDirectEditing} onClick={() => { if (isDirectEditing) { commitCodeRevision(); return; } setIsDirectEditing(true); setActiveTab(isSandboxRenderable ? 'preview' : 'code'); }} className={`min-h-11 min-w-11 rounded-[10px] px-3 text-[13px] font-semibold ${isDirectEditing ? 'origin-primary-button' : 'origin-secondary-button'}`}>✏️ {isDirectEditing ? t.finishEditing : language === 'ja' ? '編集' : 'Edit'}</button><button type="button" data-testid="artifact-action-save" aria-label={t.downloadArtifact} onClick={downloadArtifact} className="origin-secondary-button min-h-11 min-w-11 rounded-[10px] px-3 text-[13px] font-semibold">📥 {language === 'ja' ? '保存' : 'Save'}</button><div className="relative"><button type="button" data-testid="artifact-action-details" aria-label="作成物の詳細操作を開く" aria-expanded={isDetailsMenuOpen} onClick={() => { setIsDetailsMenuOpen((current) => !current); setIsExportMenuOpen(false); }} className="origin-secondary-button min-h-11 min-w-11 rounded-[10px] px-3 text-[13px] font-semibold">⋯</button>{isDetailsMenuOpen && <div data-testid="artifact-details-menu" role="menu" aria-label="作成物の詳細操作" className="origin-surface absolute right-0 top-12 z-50 grid max-h-[70vh] min-w-44 gap-0.5 overflow-y-auto rounded-xl border border-[var(--border-default)] p-1.5 shadow-xl">{isSandboxRenderable && <div className="grid gap-1 border-b border-[var(--border-default)] pb-1.5">
  <p className="origin-muted px-3 pt-1 text-[11px] font-semibold">{language === 'ja' ? '表示サイズ' : 'Preview size'}</p>
  <button type="button" data-testid="preview-viewport-375" role="menuitem" aria-label={t.phoneViewport} aria-pressed={previewViewport === '375'} onClick={() => { setIsPresentation(false); if (!isDirectEditing || syntaxResult.valid) setActiveTab('preview'); setPreviewViewport('375'); setIsDetailsMenuOpen(false); }} className="min-h-11 min-w-11 rounded-lg px-3 text-left text-[13px] font-medium transition hover:bg-black/5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent-primary)] dark:hover:bg-white/10">📱 375px</button>
  <button type="button" data-testid="preview-viewport-768" role="menuitem" aria-label={t.tabletViewport} aria-pressed={previewViewport === '768'} onClick={() => { setIsPresentation(false); if (!isDirectEditing || syntaxResult.valid) setActiveTab('preview'); setPreviewViewport('768'); setIsDetailsMenuOpen(false); }} className="min-h-11 min-w-11 rounded-lg px-3 text-left text-[13px] font-medium transition hover:bg-black/5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent-primary)] dark:hover:bg-white/10">📱 768px</button>
  <button type="button" data-testid="preview-viewport-fluid" role="menuitem" aria-label={t.fluidViewport} aria-pressed={previewViewport === 'fluid'} onClick={() => { setIsPresentation(false); if (!isDirectEditing || syntaxResult.valid) setActiveTab('preview'); setPreviewViewport('fluid'); setIsDetailsMenuOpen(false); }} className="min-h-11 min-w-11 rounded-lg px-3 text-left text-[13px] font-medium transition hover:bg-black/5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent-primary)] dark:hover:bg-white/10">💻 100%</button>
</div>}<button type="button" data-testid="artifact-action-copy" role="menuitem" aria-label={t.copyArtifact} onClick={() => void copyText(workingContent).then(() => { setCopied(true); window.setTimeout(() => setCopied(false), 2_000); })} className="min-h-11 min-w-11 rounded-lg px-3 text-left text-[13px] font-medium transition hover:bg-black/5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent-primary)] dark:hover:bg-white/10">📋 {copied ? t.copied : t.copy}</button><button type="button" data-testid="artifact-action-share" role="menuitem" aria-label={t.shareArtifact} onClick={() => { setIsDetailsMenuOpen(false); void shareArtifact(); }} className="min-h-11 min-w-11 rounded-lg px-3 text-left text-[13px] font-medium transition hover:bg-black/5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent-primary)] dark:hover:bg-white/10">📲 {shared ? language === 'ja' ? '共有済み' : 'Shared' : language === 'ja' ? '共有' : 'Share'}</button>{priorRevision && <button type="button" data-testid="artifact-visual-diff-toggle" role="menuitem" aria-label="最新と1つ前の版の変更箇所を見る" aria-pressed={isDiffInspectorOpen} onClick={() => { setIsDirectEditing(false); setIsDiffInspectorOpen((current) => !current); setIsDetailsMenuOpen(false); }} className={`min-h-11 min-w-11 rounded-[10px] px-3 text-left text-[13px] font-semibold ${isDiffInspectorOpen ? 'origin-primary-button' : 'origin-secondary-button'}`}>🔍 {isDiffInspectorOpen ? '変更箇所を閉じる' : '変更箇所を見る'}</button>}<div className="relative"><button type="button" data-testid="artifact-action-export-menu" role="menuitem" aria-label="別の形式で保存" aria-expanded={isExportMenuOpen} onClick={() => setIsExportMenuOpen((current) => !current)} className="min-h-11 min-w-11 w-full rounded-lg px-3 text-left text-[13px] font-medium transition hover:bg-black/5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent-primary)] dark:hover:bg-white/10">別の形式で保存</button>{isExportMenuOpen && <div data-testid="artifact-export-menu" role="menu" aria-label="作成物の保存形式" className="grid min-w-36 gap-1 rounded-xl border border-[var(--border-default)] p-1">{([{ key: 'html', label: 'HTML' }, { key: 'svg', label: 'SVG' }, { key: 'png', label: 'PNG' }, { key: 'markdown', label: 'Markdown' }, { key: 'json', label: 'JSON' }] as const).map((option) => <button key={option.key} type="button" data-testid={`artifact-export-${option.key}`} role="menuitem" onClick={() => void downloadArtifactFormat(option.key)} className="min-h-11 min-w-11 rounded-lg px-3 text-left text-[13px] font-medium transition hover:bg-black/5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent-primary)] dark:hover:bg-white/10">{option.label}</button>)}</div>}</div><button type="button" data-testid="artifact-action-bundle" role="menuitem" aria-label={language === 'ja' ? 'すべてをZIPで保存' : 'Save all as ZIP'} aria-busy={isBundling} onClick={() => void downloadArtifactBundle()} className="min-h-11 min-w-11 rounded-lg px-3 text-left text-[13px] font-medium transition hover:bg-black/5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent-primary)] disabled:cursor-wait disabled:opacity-60 dark:hover:bg-white/10" disabled={isBundling}>📦 {isBundling ? (language === 'ja' ? '準備中' : 'Preparing') : (language === 'ja' ? 'すべてをZIPで保存' : 'Save all as ZIP')}</button>{isSandboxRenderable && <button type="button" data-testid="presentation-mode-toggle" role="menuitem" aria-label={isPresentation ? t.exitPresentation : t.presentation} aria-pressed={isPresentation} onClick={() => { setIsDetailsMenuOpen(false); void togglePresentation(); }} className="min-h-11 min-w-11 rounded-lg px-3 text-left text-[13px] font-medium transition hover:bg-black/5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent-primary)] dark:hover:bg-white/10">▣ {isPresentation ? t.exitPresentation : t.presentation}</button>}<button type="button" role="menuitem" aria-label={isFullscreen ? t.exitFullscreenLabel : t.openFullscreen} aria-pressed={isFullscreen} onClick={() => { setIsDetailsMenuOpen(false); void toggleFullscreen(); }} className="min-h-11 min-w-11 rounded-lg px-3 text-left text-[13px] font-medium transition hover:bg-black/5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent-primary)] dark:hover:bg-white/10">{isFullscreen ? t.exitFullscreen : t.fullscreen}</button>{priorRevision && <button type="button" data-testid="artifact-restore-previous" role="menuitem" aria-label="1つ前の版をこの版に戻す" onClick={restorePreviousVersion} className="min-h-11 min-w-11 rounded-lg px-3 text-left text-[13px] font-medium transition hover:bg-black/5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent-primary)] dark:hover:bg-white/10">↶ この版に戻す</button>}{onOpenSettings && <button type="button" data-testid="artifact-open-design-settings" role="menuitem" onClick={() => { setIsDetailsMenuOpen(false); onOpenSettings(); }} className="min-h-11 min-w-11 rounded-lg px-3 text-left text-[13px] font-medium transition hover:bg-black/5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent-primary)] dark:hover:bg-white/10">🎨 {language === 'ja' ? 'デザインテーマ' : 'Design theme'}</button>}</div>}</div></div><button type="button" data-testid="artifact-close-workspace" aria-label={t.closeWorkspace} onClick={onClose} className="origin-secondary-button inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-[10px] text-lg">✕</button></div>
      </div>
      {isPresentation && <p className="sr-only" role="status">{t.presentationKeyboardHint}</p>}{isDirectEditing && isSandboxRenderable && activeTab === 'preview' && <p data-testid="artifact-direct-touch-status" role="status" className="origin-safe-note mt-2 rounded-[10px] px-3 py-2 text-[13px] font-semibold">{language === 'ja' ? '編集モード：プレビュー上の文字を直接変更できます。ソースを編集する場合は「コード」を開いてください。' : 'Edit mode: change text directly in the preview. Open Code to edit the source.'}</p>}{isOffline && <p data-testid="artifact-offline-status" role="status" className="text-[13px] text-amber-600 dark:text-amber-300">オフライン中: 端末内の作成物は閲覧・編集・保存できます。</p>}{bundleError && <p role="alert" className="text-[13px] text-[var(--danger)]">パッケージを作成できませんでした。</p>}{exportError && <p role="alert" className="text-[13px] text-[var(--danger)]">この形式でのローカル書き出しを完了できませんでした。</p>}
    </div>
    ...
    {isStreaming && !artifact.isComplete && onSteer && <form data-testid="artifact-live-steering" aria-label="作成物の方向修正" onSubmit={(event) => { event.preventDefault(); const direction = steeringPrompt.trim(); if (!direction) return; setSteeringPrompt(''); onSteer(direction); }} className="safe-area-bottom origin-surface-muted shrink-0 px-3 pt-3"><label htmlFor="artifact-live-steering-input" className="mb-2 block text-[13px] font-semibold text-[var(--accent-primary)]">⚡ {language === 'ja' ? '方向修正' : 'Adjust direction'}</label><div className="flex items-center gap-2"><input id="artifact-live-steering-input" data-testid="artifact-live-steering-input" aria-label={language === 'ja' ? '作成物の方向修正' : 'Adjust artifact direction'} value={steeringPrompt} onChange={(event) => setSteeringPrompt(event.target.value)} onKeyDown={(event) => { if ((event.nativeEvent.isComposing || event.keyCode === 229) && event.key === 'Enter') event.preventDefault(); }} placeholder={language === 'ja' ? '例：配色を落ち着いたネイビーに' : 'Example: use a calmer navy palette'} maxLength={1200} className="origin-input min-h-11 min-w-0 flex-1 rounded-[10px] border border-[var(--border-default)] bg-transparent px-3 text-base" /><button type="submit" data-testid="artifact-live-steering-submit" disabled={!steeringPrompt.trim()} className="origin-primary-button min-h-11 min-w-11 rounded-[10px] px-3 text-[13px] font-semibold disabled:opacity-50">{language === 'ja' ? '反映' : 'Apply'}</button></div></form>}
  </aside>;
};

export type OriginPersonalAppProps = { onOpenSettings?: () => void; onOpenResearch?: () => void; onOpenCoding?: () => void; onOpenCreative?: () => void; onOpenDetails?: () => void; messages?: ConversationMessage[]; sessions?: readonly ConversationSession[]; artifacts?: readonly ArtifactBlock[]; onArchiveSession?: (messages: readonly ConversationMessage[]) => void; onRestoreSession?: (session: ConversationSession) => void; onMessagesChange?: (messages: ConversationMessage[]) => void; onArtifactsChange?: (artifacts: ArtifactBlock[]) => void; resetSignal?: number; language?: OriginLanguage; designTheme?: OriginDesignTheme; embedded?: boolean };
export const App: React.FC<OriginPersonalAppProps> = ({ onOpenSettings, onOpenResearch, onOpenCoding, onOpenCreative, onOpenDetails, messages: controlledMessages, sessions = [], artifacts: controlledArtifacts, onArchiveSession, onRestoreSession, onMessagesChange, onArtifactsChange, resetSignal = 0, language = 'ja', designTheme = 'minimal', embedded = false }) => {
  const t = getTranslations(language);
  ...
  <div role="menu" aria-label={language === 'ja' ? '追加機能' : 'Additional tools'} className="origin-add-menu__panel origin-surface absolute bottom-14 right-0 z-50 grid min-w-40 gap-0.5 rounded-xl border border-[var(--border-default)] p-1.5 shadow-xl">
    ...
    {onOpenResearch && <button type="button" role="menuitem" onClick={(event) => { closeComposerMenu(event.currentTarget); onOpenResearch(); }} className="min-h-11 rounded-lg px-3 text-left text-sm font-medium transition hover:bg-black/5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent-primary)] dark:hover:bg-white/10">{language === 'ja' ? '詳しく調べる' : 'Research deeply'}</button>}
    {onOpenCoding && <button type="button" role="menuitem" onClick={(event) => { closeComposerMenu(event.currentTarget); onOpenCoding(); }} className="min-h-11 rounded-lg px-3 text-left text-sm font-medium transition hover:bg-black/5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent-primary)] dark:hover:bg-white/10">{language === 'ja' ? 'コード' : 'Code'}</button>}
    {onOpenCreative && <button type="button" role="menuitem" onClick={(event) => { closeComposerMenu(event.currentTarget); onOpenCreative(); }} className="min-h-11 rounded-lg px-3 text-left text-sm font-medium transition hover:bg-black/5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent-primary)] dark:hover:bg-white/10">{language === 'ja' ? '作成物を作る' : 'Create a deliverable'}</button>}
    {onOpenDetails && <button type="button" role="menuitem" onClick={(event) => { closeComposerMenu(event.currentTarget); onOpenDetails(); }} className="min-h-11 rounded-lg px-3 text-left text-sm font-medium transition hover:bg-black/5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent-primary)] dark:hover:bg-white/10">{language === 'ja' ? 'その他の機能' : 'More tools'}</button>}
  </div>
</details> : <button type="button" onClick={() => fileInputRef.current?.click()} aria-label={t.attachFile} className="inline-flex h-11 min-h-11 w-11 min-w-11 shrink-0 items-center justify-center rounded-full text-2xl leading-none transition hover:bg-black/5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent-primary)] dark:hover:bg-white/10">＋</button>}{isLoading ? <button type="button" aria-label={t.stopGeneration} title={t.stopGeneration} onClick={() => abortRef.current?.abort()} className="origin-danger-button inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-full p-0 text-base font-black" aria-describedby="origin-chat-guidance">■</button> : <button type="button" data-testid={messages.length ? 'send-request-button' : 'start-request-button'} aria-label={messages.length ? t.sendRequest : t.startRequest} title={messages.length ? t.sendRequest : t.startRequest} onClick={() => void handleSend()} disabled={(!inputText.trim() && !attachments.length)} className="origin-primary-button inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-full p-0 text-xl font-black disabled:cursor-not-allowed disabled:opacity-40" aria-describedby="origin-chat-guidance">↑</button>}</div>{attachments.length > 0 && <div className="origin-muted mt-2 flex flex-wrap gap-2 text-[13px]"><span className="sr-only">{t.attachedFiles}</span>{attachments.map((attachment, index) => <span key={`${attachment.name}-${index}`} className="origin-surface-muted flex items-center gap-2 rounded-[10px] px-2 py-1"><span>{t.attach}: {attachment.name}</span><button type="button" onClick={() => setAttachments((current) => current.filter((_, currentIndex) => currentIndex !== index))} aria-label={`${t.removeAttachment}: ${attachment.name}`} className="origin-secondary-button min-h-11 min-w-11 rounded-[10px] px-3 text-[13px]">✕</button></span>)}</div>}{attachmentError && <p data-testid={isSafeWaiting ? "origin-safe-waiting-state" : undefined} role="alert" aria-live={isSafeWaiting ? "assertive" : undefined} className="mt-2 text-[13px] text-[var(--danger)]">{attachmentError}</p>}<p id="origin-chat-guidance" className="sr-only">{t.keyboardGuidance}{t.dropFiles}</p></>;
  return <div className={`origin-app flex ${embedded ? "h-full w-full" : "h-[100dvh] w-screen"} overflow-hidden font-sans`}><main className="relative flex h-full min-w-0 flex-1 flex-col overflow-hidden"><header className="origin-header flex min-h-14 items-center justify-between px-3 backdrop-blur-md sm:px-4">
