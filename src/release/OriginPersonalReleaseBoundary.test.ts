...
describe('ORIGIN Personal production entrypoint', () => {
  it('ships PersonalEditionApp as the production wrapper for the shared UI', () => {
    const entrypoint = readFileSync(resolve(process.cwd(), 'src/main.tsx'), 'utf8');
    const personalEdition = readFileSync(resolve(process.cwd(), 'src/components/personal/PersonalEditionApp.tsx'), 'utf8');
    const app = readFileSync(resolve(process.cwd(), 'src/App.tsx'), 'utf8');

    expect(entrypoint).toContain("import PersonalEditionApp from './components/personal/PersonalEditionApp'");
    expect(entrypoint).toContain("import SettingsModal from './components/SettingsModal'");
    expect(entrypoint).toContain("import { usePersonalSettings } from './hooks/usePersonalSettings'");
    expect(entrypoint).toContain('<PersonalEditionApp');
    expect(entrypoint).not.toContain("from './hooks/useAppState'");

    expect(personalEdition).toContain("import App from '../../App';");
    expect(personalEdition).toContain('onOpenSettings={onOpenSettings}');
    expect(personalEdition).toContain('language={settings?.language ?? \'ja\'}');
    expect(personalEdition).toContain('designTheme={settings?.designTheme ?? \'minimal\'}');
  ...
  it('uses a Japanese-first, dependency-light document boundary', () => {
    ...
    for (const source of [document, styles, entrypoint]) { expect(source).not.toMatch(/manus-runtime|manus\.computer|debug-collector/i); expect(source).not.toMatch(/<script[^>]+src=["']https?:\/\//i); }
  });

  it('does not mount the legacy dashboard API or Mission Engine', () => {
    const serverComposition = readFileSync(resolve(process.cwd(), 'src/server/createOriginApp.ts'), 'utf8');
