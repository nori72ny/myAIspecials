...
describe('PersonalDashboard', () => {
  ...
  it('presents one clear Japanese intent-first entry point', () => {
    ...
    expect(screen.getByText('無料AIのみ。有料AIへ自動で切り替えません。')).toBeTruthy();
    expect(screen.getByText('すぐに始める')).toBeTruthy();
  ...
  it('keeps starter prompts editable instead of executing them immediately', () => {
    ...
    expect((screen.getByLabelText('実現したいことを入力') as HTMLTextAreaElement).value).toBe('候補の情報を貼り付けて、違いを比較したい');
  });

  it('offers only capabilities that the current product can truthfully support', () => {
    render(<PersonalDashboard onNavigateToChat={vi.fn()} />);
    for (const label of ['整理する', '比較する', '文章にする', '計画する']) expect(screen.getByRole('button', { name: label })).toBeTruthy();
  ...
  it('sends a trimmed request from the button and Enter key', () => {
    ...
    fireEvent.change(screen.getByLabelText('実現したいことを入力'), { target: { value: '比較表を作りたい' } });
    fireEvent.keyDown(screen.getByLabelText('実現したいことを入力'), { key: 'Enter', shiftKey: false });
  ...
  it('does not submit while Japanese IME composition is active', () => {
    ...
    fireEvent.keyDown(input, { key: 'Enter', shiftKey: false, isComposing: true, keyCode: 229 });
    expect(onNavigateToChat).not.toHaveBeenCalled();
  });

  it('does not submit an empty request and exposes the current zero-cost safety boundary', () => {
    const onNavigateToChat = vi.fn();
    render(<PersonalDashboard onNavigateToChat={onNavigateToChat} />);
  ...
  it('connects the home description and safety note to the request field', () => {
    render(<PersonalDashboard onNavigateToChat={vi.fn()} />);
    expect(screen.getByLabelText('実現したいことを入力').getAttribute('aria-describedby')).toBe('origin-home-description origin-home-safety');
  });
  ...
  it('renders the English variant when English is selected', () => {
    ...
    expect(screen.getByText('Start from a workflow')).toBeTruthy();
  });
});
