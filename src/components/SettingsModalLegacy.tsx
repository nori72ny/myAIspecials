        ...
        >
          ...
          <motion.div data-testid="settings-modal" initial={{ opacity: 0, scale: 0.97, y: 8 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.97, y: 8 }} className="origin-surface relative flex max-h-[calc(100dvh-1.5rem)] w-full max-w-lg flex-col overflow-hidden rounded-2xl border sm:max-h-[calc(100dvh-2rem)]" onPointerDown={stopPointer} style={{ overscrollBehavior: 'contain' }}>
            <div className="flex items-center justify-between border-b border-[var(--border-default)] px-5 py-4">
              ...
              <button ref={closeButtonRef} type="button" onPointerDown={(event) => { stopPointer(event); onClose(); }} data-testid="close-settings-button" aria-label={t.closeSettings} className="origin-secondary-button inline-flex h-11 w-11 items-center justify-center rounded-xl border"><X className="h-5 w-5" aria-hidden="true" /></button>
            </div>
