...
export default function SettingsModalLegacy({
  ...
  const selectedTheme = settings.selectedTheme === 'light' || settings.selectedTheme === 'dark' || settings.selectedTheme === 'system' ? settings.selectedTheme : 'system';
  const selectedDesignTheme = settings.designTheme === 'luxury' || settings.designTheme === 'glass' ? settings.designTheme : 'minimal';
  ...
  const handleImport = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    event.target.value = '';
    if (!file) return;
    try { await onImportHistory(file); setHistoryStatus({ kind: 'success', message: t.historyImported }); }
    catch (error) { setHistoryStatus({ kind: 'error', message: error instanceof Error ? error.message : t.historyImportFailed }); }
  };
  const resetHistory = () => { onResetHistory(); setHistoryStatus({ kind: 'success', message: t.historyCleared }); };
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        ...
        >
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="origin-dialog-overlay pointer-events-none absolute inset-0 backdrop-blur-[2px]" aria-hidden="true" />
          <motion.div data-testid="settings-modal" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 8 }} className="origin-surface relative flex max-h-[calc(100dvh-1.5rem)] w-full max-w-lg flex-col overflow-hidden rounded-2xl border sm:max-h-[calc(100dvh-2rem)]" onPointerDown={stopPointer} style={{ overscrollBehavior: 'contain' }}>
            <div className="flex items-center justify-between border-b border-[var(--border-default)] px-5 py-4">
              <div><h2 id="settings-title" className="text-base font-bold">{t.settings}</h2><p id="settings-description" className="origin-muted mt-1 text-[13px]">{t.settingsDescription}</p></div>
              <button ref={closeButtonRef} type="button" onPointerDown={(event) => { stopPointer(event); onClose(); }} data-testid="close-settings-button" aria-label={t.closeSettings} className="origin-secondary-button inline-flex h-12 min-h-12 w-12 min-w-12 items-center justify-center rounded-xl border"><X className="h-5 w-5" aria-hidden="true" /></button>
            </div>
            <div data-testid="settings-scroll-region" className="min-h-0 flex-1 space-y-6 overflow-y-auto overscroll-contain p-5 pb-7">
              <section aria-labelledby="language-heading" className="space-y-3"><h3 id="language-heading" className="flex items-center gap-2 text-sm font-bold"><Globe className="h-4 w-4 text-[var(--accent-primary)]" aria-hidden="true" />{t.languageHeading}</h3><div className="grid grid-cols-2 gap-2">{[{ id: 'ja', label: t.languageJapanese }, { id: 'en', label: t.languageEnglish }].map((language) => { const selected = settings.language === language.id; return <button type="button" key={language.id} onPointerDown={(event) => { stopPointer(event); update({ language: language.id as Settings['language'] }); }} aria-pressed={selected} style={{ minHeight: 48 }} className={cn('flex min-h-12 items-center justify-center gap-2 rounded-xl border px-3 text-sm font-semibold transition', selected ? 'origin-primary-button border-transparent' : 'origin-secondary-button')}>{selected && <Check className="h-4 w-4" aria-hidden="true" />}{language.label}</button>; })}</div></section>
              <section aria-labelledby="appearance-heading" className="space-y-3 border-t border-[var(--border-default)] pt-5"><h3 id="appearance-heading" className="text-sm font-bold">{t.appearanceHeading}</h3><div className="grid grid-cols-3 gap-2">{[{ id: 'light', label: t.themeLight, icon: Sun }, { id: 'dark', label: t.themeDark, icon: Moon }, { id: 'system', label: t.themeSystem, icon: Laptop }].map((theme) => { const selected = selectedTheme === theme.id; return <button type="button" key={theme.id} onPointerDown={(event) => { stopPointer(event); update({ selectedTheme: theme.id as Settings['selectedTheme'] }); }} aria-pressed={selected} style={{ minHeight: 48 }} className={cn('flex min-h-12 flex-col items-center justify-center gap-1 rounded-xl border px-2 py-1 text-xs font-semibold transition', selected ? 'origin-primary-button border-transparent' : 'origin-secondary-button')}><theme.icon className="h-4 w-4" aria-hidden="true" />{theme.label}</button>; })}</div><p className="origin-muted text-[13px] leading-5">{t.systemThemeDescription}</p></section>
              <section data-testid="origin-design-theme-section" aria-labelledby="design-theme-heading" className="space-y-3 border-t border-[var(--border-default)] pt-5"><h3 id="design-theme-heading" className="text-sm font-bold">{settings.language === 'ja' ? 'デザインテーマ' : 'Design theme'}</h3><div role="group" aria-labelledby="design-theme-heading" className="grid grid-cols-3 gap-2">{([{ id: 'minimal', label: 'Minimal' }, { id: 'luxury', label: 'Luxury' }, { id: 'glass', label: 'Glass' }] as const).map((theme) => <button type="button" key={theme.id} data-testid={`design-theme-${theme.id}`} aria-pressed={selectedDesignTheme === theme.id} onPointerDown={(event) => { stopPointer(event); update({ designTheme: theme.id }); }} style={{ minHeight: 48 }} className={cn('min-h-12 rounded-[10px] px-2 text-[13px] font-semibold transition-colors', selectedDesignTheme === theme.id ? 'origin-primary-button' : 'origin-secondary-button')}>{theme.label}</button>)}</div><p className="origin-muted text-[13px] leading-5">{settings.language === 'ja' ? '成果物のプレビューにも、同じデザインがすぐに反映されます。' : 'Artifact previews immediately reflect the same design.'}</p></section>
              <section aria-labelledby="history-heading" className="space-y-3 border-t border-[var(--border-default)] pt-5"><h3 id="history-heading" className="text-sm font-bold">{t.historyHeading}</h3><p className="origin-muted text-[13px] leading-5">{t.historyCount(messageCount)}</p><input ref={importInputRef} type="file" accept="application/json,.json" tabIndex={-1} className="sr-only" aria-label={t.historyFileLabel} onChange={(event) => void handleImport(event)} /><div className="grid grid-cols-1 gap-2 sm:grid-cols-3"><button type="button" onPointerDown={(event) => { stopPointer(event); onExportHistory(); }} className="origin-secondary-button inline-flex min-h-12 items-center justify-center gap-1.5 rounded-xl border px-3 text-xs font-semibold"><Download className="h-4 w-4" aria-hidden="true" />{t.exportHistory}</button><button type="button" onPointerDown={(event) => { stopPointer(event); importInputRef.current?.click(); }} className="origin-secondary-button inline-flex min-h-12 items-center justify-center gap-1.5 rounded-xl border px-3 text-xs font-semibold"><Upload className="h-4 w-4" aria-hidden="true" />{t.importHistory}</button><button type="button" onPointerDown={(event) => { stopPointer(event); resetHistory(); }} className="origin-danger-button inline-flex min-h-12 items-center justify-center gap-1.5 rounded-xl border px-3 text-xs font-semibold"><RotateCcw className="h-4 w-4" aria-hidden="true" />{t.clearHistory}</button></div><p role="status" aria-live="polite" className={cn('text-[13px] leading-5', historyStatus.kind === 'error' ? 'text-[var(--danger)]' : 'origin-muted')}>{historyStatus.kind === 'idle' ? t.portableHistoryNotice : historyStatus.message}</p></section>
              <details className="border-t border-[var(--border-default)] pt-5"><summary className="min-h-11 cursor-pointer py-3 text-sm font-bold">{settings.language === 'ja' ? '外部サービス連携' : 'Connected services'}</summary><p className="origin-muted -mt-1 mb-3 text-[13px] leading-5">{settings.language === 'ja' ? '必要な場合だけ設定します。通常の利用では変更不要です。' : 'Set this up only when needed. No changes are required for normal use.'}</p><McpConnectionsSettings language={settings.language} /></details>
              ...
              <details data-testid="release-identity" className="border-t border-[var(--border-default)] pt-5"><summary id="release-identity-heading" className="flex min-h-11 cursor-pointer items-center gap-2 py-3 text-sm font-bold"><Info className="h-4 w-4 text-[var(--accent-primary)]" aria-hidden="true" />{t.technicalInformation}</summary><section aria-labelledby="release-identity-heading" className="space-y-3"><div className="origin-surface-muted rounded-xl border p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div className="min-w-0"><p className="origin-muted text-[13px] font-semibold">{t.releaseId}</p>{releaseIdentity.status === 'loading' && <p role="status" className="origin-muted mt-1 text-sm">{t.releaseChecking}</p>}{releaseIdentity.status === 'unavailable' && <p role="status" className="mt-1 text-sm font-semibold text-[var(--danger)]">{t.releaseUnavailable}</p>}{releaseIdentity.status === 'ready' && <code data-testid="release-sha-value" className="mt-1 block max-w-full break-all text-sm font-semibold">{showFullReleaseSha ? releaseIdentity.sha : `${releaseIdentity.sha.slice(0, 12)}…`}</code>}</div>{releaseIdentity.status === 'ready' && <div className="flex flex-wrap gap-2"><button type="button" onPointerDown={(event) => { stopPointer(event); setShowFullReleaseSha((current) => !current); }} aria-expanded={showFullReleaseSha} className="origin-secondary-button min-h-12 rounded-xl border px-3 text-[13px] font-semibold">{showFullReleaseSha ? t.shortenReleaseId : t.showFullReleaseId}</button><button type="button" onPointerDown={(event) => { stopPointer(event); void copyReleaseSha(); }} className="origin-secondary-button inline-flex min-h-12 items-center gap-1.5 rounded-xl border px-3 text-[13px] font-semibold">{copyStatus === 'success' ? <Check className="h-4 w-4" aria-hidden="true" /> : <Copy className="h-4 w-4" aria-hidden="true" />}{copyStatus === 'success' ? t.copied : t.copy}</button></div>}</div><p aria-live="polite" className="origin-muted mt-2 text-[13px] leading-5">{copyStatus === 'error' ? t.releaseIdCopyFailed : t.releaseIdHelp}</p></div></section></details>
            </div>
            <div className="safe-area-bottom origin-surface-muted shrink-0 border-t border-[var(--border-default)] px-4 pt-4"><button type="button" onPointerDown={(event) => { stopPointer(event); onClose(); }} style={{ minHeight: 48 }} className="origin-primary-button h-12 min-h-12 w-full shrink-0 rounded-xl px-4 text-sm font-semibold">{t.close}</button></div>
          </motion.div>
        </div>
      )}
    </AnimatePresence>
