    ...
    <AnimatePresence>
      {isOpen && <div className="fixed inset-0 z-50 flex min-h-0 items-center justify-center overflow-hidden p-3 sm:p-4" role="dialog" aria-modal="true" aria-labelledby="settings-title" aria-describedby="settings-description">
        ...
        <motion.div data-testid="settings-modal" initial={{ opacity: 0, scale: 0.97, y: 8 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.97, y: 8 }} className="origin-surface relative flex max-h-[calc(100dvh-1.5rem)] w-full max-w-lg flex-col overflow-hidden rounded-2xl border sm:max-h-[calc(100dvh-2rem)]">
          <div className="flex items-center justify-between border-b border-[var(--border-default)] px-5 py-4"><div><h2 id="settings-title" className="text-base font-bold">{t.settings}</h2><p id="settings-description" className="origin-muted mt-1 text-[13px]">{t.settingsDescription}</p></div><button ref={closeButtonRef} type="button" onClick={onClose} data-testid="close-settings-button" aria-label={t.closeSettings} className="origin-secondary-button inline-flex h-11 w-11 items-center justify-center rounded-xl border"><X className="h-5 w-5" aria-hidden="true" /></button></div>
