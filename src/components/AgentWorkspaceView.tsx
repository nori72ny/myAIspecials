...
const initialSteps: AgentStep[] = [
  ...
  { id: 'execute', title: 'Execution', status: 'queued', detail: '承認後に登録済みツールだけを実行' },
];
...
export default function AgentWorkspaceView() {
    ...
    try {
      ...
      while (true) {
        ...
        for (const frame of frames) {
          ...
          try { const event = JSON.parse(data) as { type?: string; step?: AgentStep; message?: string; artifact?: string };
  ...
  return <section className="grid min-h-[calc(100vh-5rem)] grid-cols-1 gap-4 bg-slate-50 p-3 text-slate-900 dark:bg-slate-950 dark:text-slate-100 md:grid-cols-[minmax(280px,0.8fr)_minmax(0,1.4fr)] md:p-5" aria-label="Agent Workspace">
    ...
    <main className="min-h-0 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900"><div className="flex items-center justify-between border-b border-slate-200 px-4 py-3 dark:border-slate-800"><div><p className="text-xs font-bold uppercase tracking-widest text-cyan-600">Live Artifact Sandbox</p><h2 className="font-bold">Preview / Working Artifact</h2></div><span className="rounded-full bg-slate-100 px-3 py-1 text-xs dark:bg-slate-800">$0 · permission gated</span></div><pre className="h-[calc(100%-4.5rem)] overflow-auto whitespace-pre-wrap p-5 font-mono text-sm leading-6">{artifact}</pre></main>
  </section>;
}
