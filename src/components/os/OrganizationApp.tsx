...
export default function OrganizationApp({ settings, updateSettings }: OrganizationAppProps) {
  ...
  // Browser credential entry is fail-closed.
  ...
  // Browser OAuth client-secret handling is also disabled.
  const handleConnectOAuth = async () => {
    setCustomClientSecret("");
    setIsGitHubConnecting(false);
    setGithubError(isEn
      ? "Legacy browser OAuth is disabled. Use the approved server-side GitHub integration."
      : "旧ブラウザOAuthは無効です。承認済みのサーバー側GitHub連携を使用してください。");
  };

  // Disconnect the inert legacy state without touching browser credential storage.
