...
function VerificationPanel({ result, state }: { result: CodingJobResult | null; state: ResultDetailsState }) {
  ...
  return <section className="rounded-2xl border border-slate-200 bg-white/70 p-4 dark:border-slate-800 dark:bg-slate-900/50" aria-labelledby="coding-verification-title">
    <div className="mb-3 flex items-center justify-between gap-3">
      ...
      {result && <span className="text-xs text-slate-500">repair rounds: {result.repairRounds}</span>}
    </div>
...
function DiffPanel({ result, state, changedPaths }: { result: CodingJobResult | null; state: ResultDetailsState; changedPaths: string[] }) {
  return <section className="rounded-2xl border border-slate-200 bg-white/70 p-4 dark:border-slate-800 dark:bg-slate-900/50" aria-labelledby="coding-diff-title">
    <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
      <h2 id="coding-diff-title" className="font-bold">Diff / Changed files</h2>
      <span className="text-xs text-slate-500">{changedPaths.length} file{changedPaths.length === 1 ? '' : 's'}</span>
    </div>
...
export default function CodingJobWorkspaceV14() {
  ...
  const startJob = useCallback(async () => {
    ...
    if (!credential) {
      setError('CODING_JOB_AUTHENTICATION_REQUIRED');
    ...
    try {
      const response = await fetch('/api/coding/v1.4/jobs', {
        method: 'POST',
        headers: authHeaders(credential),
  ...
  const cancelJob = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await fetch(`/api/coding/v1.4/jobs/${encodeURIComponent(job.jobId)}`, {
        method: 'DELETE',
    ...
    <div className="mx-auto grid max-w-[1600px] gap-4 xl:grid-cols-[minmax(300px,0.72fr)_minmax(0,1.65fr)]">
      <aside className="origin-workspace rounded-2xl p-4 xl:sticky xl:top-4 xl:self-start">
        <div className="flex items-start justify-between gap-3">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.18em] text-indigo-500">V1.4 · Agentic Coding OS</p>
            <h1 className="mt-1 text-xl font-black">Coding Job</h1>
            <p className="mt-2 text-xs leading-5 text-slate-500">自然言語で変更を依頼し、隔離workerで探索・編集・検証・修復を実行します。</p>
          </div>
          <span className="rounded-full bg-emerald-100 px-2 py-1 text-[10px] font-black text-emerald-800 dark:bg-emerald-950 dark:text-emerald-300">$0 only</span>
        </div>
        ...
        <button type="button" onClick={() => void startJob()} disabled={!ready || !goal.trim() || busy || Boolean(job && ACTIVE.has(job.status))} className="origin-primary-button mt-3 min-h-11 w-full rounded-xl px-4 font-bold text-white disabled:cursor-not-allowed disabled:opacity-50">
          {busy ? 'Processing…' : job && ACTIVE.has(job.status) ? 'Job running' : 'Start coding job'}
        </button>
        ...
        {job && <div className="mt-4 space-y-3 rounded-xl border border-slate-200 p-3 dark:border-slate-800" aria-live="polite">
          ...
          <dl className="grid grid-cols-[auto_1fr] gap-x-3 gap-y-1 text-[10px]">
            ...
            <dt className="text-slate-500">Updated</dt><dd>{formatTime(job.updatedAt)}</dd>
            <dt className="text-slate-500">Result</dt><dd className="break-all font-mono">{job.resultCode ?? 'pending'}</dd>
          </dl>
      ...
      <main className="min-w-0 space-y-4">
        <section className="origin-workspace rounded-2xl p-4" aria-labelledby="coding-progress-title">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div><p className="text-xs font-bold uppercase tracking-[0.18em] text-slate-500">Execution</p><h2 id="coding-progress-title" className="mt-1 text-lg font-black">Progress & evidence</h2></div>
        ...
        <section className="rounded-2xl border border-slate-200 bg-white/70 p-4 text-xs leading-5 text-slate-500 dark:border-slate-800 dark:bg-slate-900/50">
          <strong className="text-slate-700 dark:text-slate-200">Safety boundary:</strong> UIからrepository/ref/pathは指定できません。targetはserver-ownedの <code>origin:self</code> に固定され、Git公開・デプロイはこのV1.4経路では実行されません。
        </section>
      </main>
    </div>
  </section>;
