...
describe('safeRepositoryReader', () => {
  ...
  it('keeps repository access inside the originally opened directory when a child is replaced by a symlink', async () => {
    ...
    try {
      ...
      await expect(readRepositoryFile(root, 'src/app.ts')).rejects.toThrow();
