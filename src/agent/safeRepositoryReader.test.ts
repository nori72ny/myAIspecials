...
describe('safeRepositoryReader', () => {
  ...
  it('rejects a deterministic child-directory replacement by a symlink before file access', async () => {
    ...
    await fs.promises.rename(source, movedSource); await fs.promises.symlink(outside, source, 'dir');
    await expect(readRepositoryFile(root, 'src/app.ts')).rejects.toThrow('SYMLINK_PATH_BLOCKED');
