...
describe('completionVerificationGate', () => {
  it('requires typecheck, lint, test, and build before success', async () => {
    vi.mocked(runVerification)
      ...
      .mockResolvedValueOnce({ ok: true, kind: 'lint', exitCode: 0, timedOut: false, stdout: '', stderr: '', durationMs: 1 })
    ...
    expect(result.checks.map((check) => check.kind)).toEqual(['typecheck', 'lint', 'test', 'build']);
  ...
  it('fails closed at the first failed verification', async () => {
    vi.mocked(runVerification)
      .mockResolvedValueOnce({ ok: true, kind: 'typecheck', exitCode: 0, timedOut: false, stdout: '', stderr: '', durationMs: 1 })
      .mockResolvedValueOnce({ ok: false, kind: 'lint', exitCode: 1, timedOut: false, stdout: '', stderr: 'failed', durationMs: 2 });

    const result = await verifyBeforeReportingCompletion('/repo');
    expect(result.ok).toBe(false);
    expect(result.failedKind).toBe('lint');
