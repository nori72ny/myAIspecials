...
const request = buildOriginExecutionPlan(
...
if (!request.ok) throw new Error("security regression fixture could not build an execution plan");
...
const openRouterSuccess = (overrides: Record<string, unknown> = {}) => ({
  id: "test",
...
describe("originProviderClient security regressions", () => {
  it("fails closed on upstream 402 and never falls through to another provider", async () => {
    const fetchMock = vi.fn(async () => new Response("payment required: secret", { status: 402 }));
    await expect(executeOriginProvider(
      executionRequest,
      { OPENROUTER_API_KEY: "synthetic-key", GEMINI_API_KEY: "synthetic-gemini-key" },
      fetchMock as unknown as OriginFetch,
    )).rejects.toMatchObject({ code: "PROVIDER_POLICY_VIOLATION", retryable: false });
    expect(fetchMock).toHaveBeenCalledTimes(1);
  });

  it("rejects a successful response whose served model is not the approved free model", async () => {
    const fetchMock = vi.fn(async () => new Response(JSON.stringify(openRouterSuccess({ model: "paid-model" })), {
      status: 200,
  ...
  it("preserves Retry-After without exposing upstream response content", async () => {
    const body = "Authorization: Bearer upstream-secret-value";
    const fetchMock = vi.fn(async () => new Response(body, {
      status: 429,
      headers: { "Retry-After": "60" },
    }));
    const error = await executeOriginProvider(
      executionRequest,
      { OPENROUTER_API_KEY: "synthetic-key" },
      fetchMock as unknown as OriginFetch,
  ...
  it("falls back from a rate-limited OpenRouter request to Gemini's approved free model", async () => {
    const fetchMock = vi.fn()
      .mockResolvedValueOnce(new Response("rate limited", { status: 429 }))
      .mockResolvedValueOnce(new Response(JSON.stringify({
        candidates: [{ content: { parts: [{ text: "gemini safe response" }] } }],
  ...
  it("rejects non-zero cost evidence even when the routing evidence looks valid", () => {
    const result = {
      text: "x",
      actualCostUsd: 0,
      providerDataPolicy: executionRequest.plan.providerDataPolicy,
      routingEvidence: {
