...
async function executeWithRetry(executeFn, request) {
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    ...
    try {
      ...
      const code = err?.code ?? "";
      const is429or503 = status === 429 || status === 503 || code === "PROVIDER_RATE_LIMITED" || (code === "PROVIDER_UNAVAILABLE" && (status === 503 || status === 502));
...
export function createOriginChatRouter(options: OriginChatRouterOptions = {}) {
  ...
  router.post("/api/chat", async (req, res) => {
    ...
    if (isOriginWeatherRequest(lastUserMessage)) {
      ...
      if (!hasOriginWeatherLocation(lastUserMessage, body.userLocation)) { const content = isEnglish ? "Which location would you like to know the weather for?" : "どの地域の天気をお調べしますか？"; const reason = "地域確認のため外部AIを呼びませんでした。"; return res.json({ content, answer: answerEnvelope(content, isEnglish ? "en" : "ja", "not-required", reason), routing: applicationRouting(requestId, reason) }); }
    ...
    if (currentInformationRequired) {
      ...
      const content = futureReleaseInformationRequired ? futureAiDirectionGuidance(isEnglish) : (isEnglish ? "ORIGIN cannot verify current information in this release because live search is not connected. It will not answer from potentially outdated knowledge." : "この版では最新情報を確認する検索機能が接続されていないため、古い可能性がある知識だけでは回答しません。");
      const reason = futureReleaseInformationRequired ? (isEnglish ? "Live search is not connected. No specific future product was verified; only clearly labeled general trends were provided." : "最新情報の検索機能が未接続のため、個別製品は確認せず、一般的な技術動向だけを明示して回答しました。") : (isEnglish ? "Live search is not connected, so current facts were not verified." : "最新情報の検索機能が未接続のため、現在の事実確認を実施しませんでした。");
      const limitations = [futureReleaseInformationRequired ? (isEnglish ? "No specific upcoming product name or release date was retrieved or checked." : "今後登場する個別製品名や公開時期は取得・確認していません。") : (isEnglish ? "Current facts, prices, news, and other time-sensitive information were not retrieved or checked." : "現在の事実、料金、ニュースなど、時点に依存する情報は取得・確認していません。")];
      const nextActions = [futureReleaseInformationRequired ? (isEnglish ? "After live search is connected, add a dated release list verified against primary sources." : "ライブ検索接続後、確認日付きで一次情報を照合した公開予定一覧を追加します。") : (isEnglish ? "Paste the relevant text from an official source and ORIGIN can organize or compare that supplied content." : "公式情報の本文または必要部分を貼り付けると、その内容を整理・比較できます。")];
    ...
    try {
      ...
      let result: OriginProviderExecutionResult; result = await executeWithRetry(execute, providerRequest); assertOriginZeroCostExecutionResult(result, planningResult.plan.modelId);
      ...
      if (error instanceof OriginProviderError) {
        ...
        if (shouldRetryProvider(error)) {
          const isEnglish = !/[ぁ-んァ-ヶ一-龠]/.test(lastUserMessage); const content = isEnglish ? "Free AI capacity is temporarily busy. ORIGIN is keeping the $0 cost policy and safe synchronization active. Please try again in a few seconds." : "現在無料APIの利用が一時的に集中しています。費用0円ポリシーを維持したまま安全に同期中ですので、数秒おいて再度お試しください。"; const reason = isEnglish ? "All currently available zero-cost provider retry paths were exhausted; no paid provider was selected." : "無料プロバイダーの再試行経路を使い切りました。有料プロバイダーへは切り替えず、0円ポリシーを維持しました。";
          return res.status(200).json({ content, answer: answerEnvelope(content, isEnglish ? "en" : "ja", "not-run", reason), routing: { ...applicationRouting(requestId, reason, "not-run"), providerAttempts: providerRetryAttempted ? 2 : 1, retryAttempted: providerRetryAttempted } });
        }
        const providerError = error as OriginProviderError;
        return res.status(providerError.status).json({ code: providerError.code, message: providerError.message, retryable: providerError.retryable, retryAfterSeconds: providerError.retryAfterSeconds, diagnostic: providerError.diagnostic, requestId, retryAttempted: providerRetryAttempted });
      }
      ...
      return res.status(500).json({ code: "PROVIDER_INTERNAL_ERROR", message: "無料AIとの通信に失敗しました。", retryable: true, requestId });
    }
  });
  return router;
}