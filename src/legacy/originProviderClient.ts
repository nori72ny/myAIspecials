...
/**
 * Zero-cost execution is fail-closed: a provider/model is trusted only when it is
 * explicitly represented by this allowlist. The OpenRouter entry additionally
 * requires the model's :free suffix, preventing a paid model from being accepted
 * merely because an attacker supplied a matching string elsewhere in the route.
 */
...
function rejectZeroCost(message: string, code: OriginProviderErrorCode = "PROVIDER_POLICY_VIOLATION"): never {
...
function assertFiniteZeroCost(value: unknown, field: string): asserts value is 0 {
  if (typeof value !== "number" || !Number.isFinite(value) || value < 0 || Math.abs(value) > Number.EPSILON) {
    rejectZeroCost(`${field} が厳格な$0ポリシーを満たさないため、回答を破棄しました。`);
  }
}
...
export function assertOriginZeroCostExecutionResult(result: OriginProviderExecutionResult, expectedModel: string = ORIGIN_OPENROUTER_FREE_MODEL): void {
  ...
  assertFiniteZeroCost(actualCost, "actualCostUsd");
  assertFiniteZeroCost(usageCost, "usage.costUsd");
  ...
  if (typeof e.requestedModel !== "string" || typeof e.servedModel !== "string") {
    rejectZeroCost("要求モデルまたは提供モデルを確認できないため、回答を返しません。", "PROVIDER_ROUTING_UNVERIFIED");
  }
  if (!isAllowedModel(provider, e.servedModel)) {
    rejectZeroCost("許可されていない無料モデル、または無料モデルとして確認できないモデルを検出したため、回答を返しません。", "PROVIDER_ROUTING_UNVERIFIED");
  }
  if (expectedModel !== ORIGIN_OPENROUTER_FREE_MODEL || !isAllowedModel("openrouter", expectedModel)) {
    rejectZeroCost("要求モデルがORIGINの許可済み無料モデルではないため、回答を返しません。", "PROVIDER_ROUTING_UNVERIFIED");
  }
  if (e.requestedModel !== expectedModel) {
    rejectZeroCost("要求モデルの実行証跡がORIGINの固定無料モデルと一致しないため、回答を返しません。", "PROVIDER_ROUTING_UNVERIFIED");
  }
  if (e.attempt !== 1) rejectZeroCost("未許可の実行試行番号を検出したため、回答を返しません。", "PROVIDER_ROUTING_UNVERIFIED");

  const isFallback = e.fallbackUsed === true;
  if (isFallback) {
    if (e.strategy !== "zero-cost-failover") rejectZeroCost("フォールバック実行の戦略を確認できないため、回答を返しません。", "PROVIDER_ROUTING_UNVERIFIED");
    return;
  }

  if (e.fallbackUsed !== false || e.strategy !== "fixed-free-model" || provider !== "openrouter" || e.servedModel !== expectedModel) {
    rejectZeroCost("固定無料モデルの応答証跡を確認できないため、回答を返しません。", "PROVIDER_ROUTING_UNVERIFIED");
  }
}
...
function normalizeMessages(messages: OriginChatMessage[], systemInstruction: string) {
  return [{ role: "system", content: systemInstruction }, ...messages.map((m) => ({
    role: m.role === "ai" || m.role === "assistant" || m.role === "model" ? "assistant" : "user", content: m.content,
  }))];
}
function extractText(content: unknown): string {
  if (typeof content === "string") return content.trim();
  if (!Array.isArray(content)) return "";
  return content.filter((p): p is { type?: string; text?: string } => typeof p === "object" && p !== null)
    .filter((p) => p.type === "text" && typeof p.text === "string").map((p) => p.text!.trim()).filter(Boolean).join("\n\n").trim();
}
function mergeContinuation(previous: string, continuation: string): string {
  ...
  for (let n = Math.min(a.length, b.length, 20); n >= 1; n -= 1) {
    const aa = a.slice(-n).map((x) => x.trim().toLocaleLowerCase()); const bb = b.slice(0, n).map((x) => x.trim().toLocaleLowerCase());
    if (aa.every(Boolean) && aa.every((x, i) => x === bb[i])) return [...a, ...b.slice(n)].join("\n").trim();
  }
  return `${left}\n\n${right}`.trim();
}
function parseRetryAfterSeconds(v: string | null): number | undefined {
  if (!v) return undefined; const n = Number(v); if (Number.isFinite(n) && n > 0) return Math.ceil(n);
  const t = Date.parse(v); return Number.isFinite(t) ? Math.max(1, Math.ceil((t - Date.now()) / 1000)) : undefined;
}
...
function validatePlan(plan: OriginExecutionPlan): void {
  if (!plan.freeOnly || plan.estimatedCostUsd !== 0 || plan.providerDataPolicy.allowProviderFallbacks !== true || plan.providerDataPolicy.dataCollection !== "deny" || plan.providerDataPolicy.requireZeroDataRetention !== false) {
    throw new OriginProviderError("PROVIDER_POLICY_VIOLATION", "0ドル固定ポリシーに適合しない実行計画は使用できません。", 400, false);
  }
...
function freeUsage(cost: unknown): 0 {
  if (typeof cost !== "number" || !Number.isFinite(cost) || cost < 0 || Math.abs(cost) > Number.EPSILON) {
    throw new OriginProviderError("PROVIDER_POLICY_VIOLATION", "無料実行で0ドル以外、または検証不能な利用額が報告されたため、回答を破棄しました。", 502, false);
  }
  return 0;
}
...
async function executeOpenRouter(request: OriginProviderExecutionRequest, apiKey: string, fetchImpl: OriginFetch): Promise<OriginProviderExecutionResult> {
  const controller = new AbortController(); const timeout = setTimeout(() => controller.abort(), request.plan.timeoutMs);
  try {
...
function shouldFailover(error: unknown): boolean { return error instanceof OriginProviderError && error.retryable && (error.code === "PROVIDER_RATE_LIMITED" || error.code === "PROVIDER_UNAVAILABLE" || error.code === "PROVIDER_TIMEOUT"); }
...
export async function executeOriginProvider(request: OriginProviderExecutionRequest, env: NodeJS.ProcessEnv = process.env, fetchImpl: OriginFetch = fetch): Promise<OriginProviderExecutionResult> {
  ...
  if (openRouterKey) {
    try { return await executeOpenRouter(request, openRouterKey, fetchImpl); } catch (e) { if (!shouldFailover(e)) throw e; failures.push(e as OriginProviderError); }
  }
  if (geminiKey) {
    try { return await executeGemini(request, geminiKey, fetchImpl); } catch (e) { if (!shouldFailover(e)) throw e; failures.push(e as OriginProviderError); }
  }
  if (groqKey) {
    try { return await executeGroq(request, groqKey, fetchImpl); } catch (e) { if (!shouldFailover(e)) throw e; failures.push(e as OriginProviderError); }
  }
  const last = failures.at(-1);
  throw last ?? new OriginProviderError("PROVIDER_UNAVAILABLE", "無料AIを現在利用できません。", 503, true);
}
