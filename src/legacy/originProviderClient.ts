...
function freeUsage(cost: unknown): 0 { if (cost === undefined || cost === null) return 0; if (typeof cost !== "number" || !Number.isFinite(cost) || cost !== 0) throw new OriginProviderError("PROVIDER_POLICY_VIOLATION", "無料実行で0ドル以外の利用額が報告されたため、回答を破棄しました。", 502, false); return 0; }
...
async function executeOpenRouter(request: OriginProviderExecutionRequest, apiKey: string, fetchImpl: OriginFetch): Promise<OriginProviderExecutionResult> {
  ...
  try {
    ...
    for (let segment = 0; segment < MAX_COMPLETION_SEGMENTS; segment += 1) {
      ...
      const model = data.model; if (model !== ORIGIN_OPENROUTER_FREE_MODEL) throw new OriginProviderError("PROVIDER_ROUTING_UNVERIFIED", "OpenRouterの固定無料モデル利用を確認できませんでした。", 502, false);
