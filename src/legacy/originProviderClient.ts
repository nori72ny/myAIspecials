...
async function openrouter(request: OriginProviderExecutionRequest, key: string, fetchImpl: OriginFetch, provider: AllowedZeroCostProvider): Promise<OriginProviderExecutionResult> {
  ...
  for (let index = 0; index < MAX_SEGMENTS; index += 1) {
    const response = await requestFetch(fetchImpl, request, key, messages); if (!response.ok) throw http(response.status, retryAfter(response.headers.get("Retry-After"))); const data = await json(response) as { model?: unknown; choices?: Array<{ message?: { content?: unknown }; finish_reason?: unknown; error?: { metadata?: { error_type?: unknown } } }>; error?: { metadata?: { error_type?: unknown } }; usage?: { prompt_tokens?: number; completion_tokens?: number; total_tokens?: number; cost?: number; cost_details?: { upstream_inference_cost?: unknown }; is_byok?: unknown }; billing_tier?: unknown; is_free?: unknown; pricing?: { prompt?: unknown; completion?: unknown } };
    ...
    const servedModel = typeof data.model === "string" && data.model.trim() ? data.model.trim() : request.plan.modelId; if (!allowed(provider, servedModel)) throw new OriginProviderError("PROVIDER_ROUTING_UNVERIFIED", "OpenRouter無料モデルを確認できません。", 502, false, undefined, { upstreamErrorType: `served-model:${servedModel}` }); assertBillingMetadata(data);
