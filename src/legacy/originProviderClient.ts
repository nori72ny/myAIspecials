...
export const ALLOWED_ZERO_COST_MODELS = {
  openrouter: [ORIGIN_OPENROUTER_FREE_MODEL],
} as const;

const IDS: Record<string, AllowedZeroCostProvider> = {
...
export function resetOriginProviderCooldownForTests(): void {
  for (const provider of ALLOWED_ZERO_COST_PROVIDERS) delete cooldown[provider];
}
...
const pid = (value: unknown): AllowedZeroCostProvider | null => {
  ...
  return (
    IDS[value] ??
    (ALLOWED_ZERO_COST_PROVIDERS.includes(value as AllowedZeroCostProvider)
      ? (value as AllowedZeroCostProvider)
      : null)
  );
};
...
const allowed = (provider: AllowedZeroCostProvider, model: unknown): model is string =>
  typeof model === "string" &&
  (ALLOWED_ZERO_COST_MODELS[provider] as readonly string[]).includes(model);

const mark = (provider: AllowedZeroCostProvider): void => {
  cooldown[provider] = Date.now() + COOLDOWN;
};

const isCooling = (provider: AllowedZeroCostProvider): boolean => {
  const until = cooldown[provider] ?? 0;
  if (until <= Date.now()) {
    delete cooldown[provider];
    return false;
  }
  return true;
};

function fail(message: string, code: OriginProviderErrorCode = "PROVIDER_POLICY_VIOLATION"): never {
...
function assertBillingMetadata(payload: unknown): void {
  ...
  if (
    data.billing_tier !== undefined &&
    String(data.billing_tier).toLowerCase() !== "free"
  ) {
    fail("有料の課金ティアが検出されました。", "PROVIDER_POLICY_VIOLATION");
  }
  if (data.is_free === false) {
    fail("無料モデルではない証跡が検出されました。", "PROVIDER_POLICY_VIOLATION");
  }
  ...
  if (data.usage?.is_byok === true) {
    fail("BYOK課金経路は$0境界で許可されません。", "PROVIDER_POLICY_VIOLATION");
  }
}
...
export function assertOriginZeroCostExecutionResult(
  ...
  if (!result || typeof result !== "object") {
    fail("実行結果を検証できません。", "PROVIDER_COST_UNVERIFIED");
  }
  ...
  if (!evidence || !provider || !allowed(provider, evidence.servedModel)) {
    fail("許可された無料Provider/Modelの証跡を確認できません。", "PROVIDER_ROUTING_UNVERIFIED");
  }
  if (expectedModel && evidence.requestedModel !== expectedModel) {
    fail("要求モデルが一致しません。", "PROVIDER_ROUTING_UNVERIFIED");
  }
  if (evidence.attempt !== 1) {
    fail("不正な試行番号です。", "PROVIDER_ROUTING_UNVERIFIED");
  }
  if (
    evidence.fallbackUsed ||
    evidence.strategy !== "adaptive-primary" ||
    evidence.requestedModel !== evidence.servedModel
  ) {
    fail("Provider fallback またはPrimary証跡が不正です。", "PROVIDER_ROUTING_UNVERIFIED");
  }
  if (expectedProvider && pid(expectedProvider) !== provider) {
    fail("Providerが一致しません。", "PROVIDER_ROUTING_UNVERIFIED");
  }
}
...
function mergeContinuation(previous: string, continuation: string): string {
  ...
  for (let length = Math.min(left.length, right.length, 500); length >= 20; length -= 1) {
  ...
  for (let length = Math.min(previousLines.length, continuationLines.length, 20); length >= 1; length -= 1) {
    const a = previousLines.slice(-length).map((line) => line.trim().toLocaleLowerCase());
    const b = continuationLines.slice(0, length).map((line) => line.trim().toLocaleLowerCase());
    if (a.every(Boolean) && a.every((value, index) => value === b[index])) {
      return [...previousLines, ...continuationLines.slice(length)].join("\n").trim();
    }
  }
  return `${left}\n\n${right}`.trim();
}
...
function evidence(
  ...
  _provider: AllowedZeroCostProvider,
  servedModel: string,
...
async function openrouter(
  ...
  key: string,
  fetchImpl: OriginFetch,
  provider: AllowedZeroCostProvider,
): Promise<OriginProviderExecutionResult> {
  let messages = msgs(request.messages, request.systemInstruction);
  let output = "";
  let promptTokens = 0;
  ...
  for (let index = 0; index < MAX_SEGMENTS; index += 1) {
    const response = await requestFetch(fetchImpl, request, key, messages);
    ...
    if (errorType === "provider_overloaded" || errorType === "provider_unavailable") {
      throw new OriginProviderError(
        ...
        true,
        undefined,
        { upstreamErrorType: String(errorType) },
      );
    }

    const servedModel = data.model;
    if (!allowed(provider, servedModel)) {
      throw new OriginProviderError(
        "PROVIDER_ROUTING_UNVERIFIED",
        "OpenRouter無料モデルを確認できません。",
        502,
        false,
      );
    }
    ...
    const part = text(data.choices?.[0]?.message?.content);
    if (!part) {
      throw new OriginProviderError(
        "PROVIDER_INVALID_RESPONSE",
    ...
    if (data.choices?.[0]?.finish_reason !== "length") {
      const result: OriginProviderExecutionResult = {
        ...
        routingEvidence: evidence(request, provider, String(servedModel)),
        usage: {
          promptTokens,
          completionTokens,
          totalTokens,
    ...
    messages = [
      ...msgs(request.messages, request.systemInstruction),
      { role: "assistant", content: output },
      {
        role: "user",
        content: "直前の回答が出力上限で途切れました。途切れた箇所から最後まで不足部分だけを続けてください。",
      },
    ];
  }
...
async function requestFetch(
  ...
  request: OriginProviderExecutionRequest,
  key: string,
  messages: ReturnType<typeof msgs>,
): Promise<Response> {
...
async function requestWithRetry(
  ...
  return request(fetchImpl, input, init);
}
...
export async function executeOriginProvider(
  ...
  try {
    return await openrouter(providerRequest, key, fetchImpl, primary);
  } catch (error) {
    if (error instanceof OriginProviderError && error.retryable) mark(primary);
    if (error instanceof OriginProviderError) throw error;
    throw new OriginProviderError(
      "PROVIDER_INTERNAL_ERROR",
      "無料AIとの通信に失敗しました。",
