...
s = main.read_text()
anchor = "function loadStoredSessions(): ConversationSession[] { try { const raw = window.localStorage.getItem(SESSION_STORAGE_KEY); if (!raw) return []; const parsed = JSON.parse(raw) as unknown; if (!Array.isArray(parsed)) return []; return parsed.slice(0, 24).flatMap((candidate, index) => { if (!candidate || typeof candidate !== 'object') return []; const source = candidate as Partial<ConversationSession>; if (typeof source.id !== 'string' || typeof source.title !== 'string' || typeof source.createdAt !== 'number' || !Array.isArray(source.messages)) return []; try { return [{ id: source.id.slice(0, 128) || `session-${index}`, title: source.title.slice(0, 120), createdAt: source.createdAt, messages: parseImportedHistory({ messages: source.messages }) }]; } catch { return []; } }); } catch { return []; } }\n"
helper = anchor + "function loadLegacySnapshot(): OriginPersistedSnapshot {\n  let updatedAt = 0;\n  try {\n    const raw = window.localStorage.getItem(HISTORY_STORAGE_KEY);\n    if (raw) {\n      const parsed = JSON.parse(raw) as { updatedAt?: unknown };\n      if (typeof parsed.updatedAt === 'number' && Number.isFinite(parsed.updatedAt)) updatedAt = Math.max(1, parsed.updatedAt);\n      else updatedAt = 1;\n    }\n  } catch { /* loadStoredHistory already fails closed. */ }\n  const sessions = loadStoredSessions();\n  if (sessions.length > 0 && updatedAt === 0) updatedAt = 1;\n  return { version: 1, messages: loadStoredHistory(), sessions, artifacts: [], updatedAt };\n}\n"
s = replace_once(s, anchor, helper, "legacy snapshot helper")
old_effect = "  useEffect(() => { let active = true; const legacy = snapshotFromState(messages, sessions, []); const cancelIdle = scheduleIdle(() => { void migrateOriginLegacySnapshot(originIndexedDbAdapter, legacy, () => { window.localStorage.removeItem(HISTORY_STORAGE_KEY); window.localStorage.removeItem(SESSION_STORAGE_KEY); }).then((result) => { if (!active) return; if (result.source === 'indexeddb' && result.snapshot) { try { setMessages(parseImportedHistory({ messages: result.snapshot.messages })); } catch { setMessages([]); } setSessions(loadSessionsFromSnapshot(result.snapshot.sessions)); setArtifacts(parseStoredArtifacts(result.snapshot.artifacts)); } setStorageHealth(result.writeResult && result.writeResult !== 'saved' ? result.writeResult : 'ready'); }); }); return () => { active = false; cancelIdle(); }; }, []);"
new_effect = "  useEffect(() => { let active = true; const cancelIdle = scheduleIdle(() => { const legacy = loadLegacySnapshot(); void migrateOriginLegacySnapshot(originIndexedDbAdapter, legacy, () => { const current = loadLegacySnapshot(); if (current.updatedAt <= legacy.updatedAt) { window.localStorage.removeItem(HISTORY_STORAGE_KEY); window.localStorage.removeItem(SESSION_STORAGE_KEY); } }).then((result) => { if (!active) return; if ((result.source === 'indexeddb' || result.source === 'migrated') && result.snapshot) { try { setMessages(parseImportedHistory({ messages: result.snapshot.messages })); } catch { setMessages([]); } setSessions(loadSessionsFromSnapshot(result.snapshot.sessions)); setArtifacts(parseStoredArtifacts(result.snapshot.artifacts)); } setStorageHealth(result.writeResult && result.writeResult !== 'saved' ? result.writeResult : 'ready'); }); }); return () => { active = false; cancelIdle(); }; }, []);"
s = replace_once(s, old_effect, new_effect, "migration effect")
main.write_text(s)
