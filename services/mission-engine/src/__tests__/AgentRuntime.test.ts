...
describe("=== Agent Runtime (Version 1 Core) Unit Tests ===", () => {
  ...
  describe("1. PromptBuilder", () => {
    ...
    it("builds detailed user prompts containing mission, task, memory, and tool history", () => {
      ...
      expect(userPrompt).toContain("Create a clean React app"); expect(userPrompt).toContain("Has headers"); expect(userPrompt).toContain("Design and write the core component layouts"); expect(userPrompt).toContain("Thought about layouts"); expect(userPrompt).toContain("CalculatorTool"); expect(userPrompt).toContain("実行結果: 4");
    });
  });
  ...
  describe("2. ToolExecutor", () => {
    ...
    it("fails closed for WebTool query execution when live network search is disabled", async () => {
      const web = new WebTool(); const res = await web.execute({ query: "Clean Architecture guidelines" }); expect(res.success).toBe(false); expect(res.error).toContain("WEB_SEARCH_UNAVAILABLE");
    });
  ...
  describe("3. OutputValidator", () => {
    it("validates JSON payloads and strips markdown code blocks", () => { const res = OutputValidator.validate("```json\n{\"status\": \"ok\"}\n```", "JSON"); expect(res.isValid).toBe(true); expect(res.parsedContent.status).toBe("ok"); });
    it("detects invalid markdown strings when expecting Markdown format", () => { expect(OutputValidator.validate("Just regular plain text output with no headers.", "Markdown").isValid).toBe(false); });
    it("treats any non-empty string as valid text", () => { expect(OutputValidator.validate("Hello world", "Text").isValid).toBe(true); });
  });

  describe("4. QualityChecker", () => {
    it("flags potential hallucinations based on template placeholders", () => { const issues = QualityChecker.check("Here is the completed code. TODO: implement login details [Insert API Key]", "Text", []); expect(issues.some(i => i.type === "Hallucination" && i.description.includes("TODO:"))).toBeDefined(); expect(issues.some(i => i.type === "Hallucination" && i.description.includes("[Insert"))).toBeDefined(); });
    it("flags missing data against required success criteria", () => { const issues = QualityChecker.check("Created components with headers.", "Text", ["Has headers", "Has checklists"]); expect(issues.some(i => i.type === "MissingData" && i.description.includes("Has checklists"))).toBeDefined(); });
  });

  describe("5. ReflectionEngine", () => {
    it("passes good content", () => { const reflection = ReflectionEngine.reflect("No issues here. Has headers and checklists and no compilation errors.", "Text", ["Has headers", "Has checklists"]); expect(reflection.passed).toBe(true); expect(reflection.score).toBeGreaterThanOrEqual(70); expect(reflection.feedback).toContain("合格"); });
    it("fails low quality content", () => { const reflection = ReflectionEngine.reflect("Wait, here is the result template [Insert Key] and TODO: code.", "JSON", ["Has checklists"]); expect(reflection.passed).toBe(false); expect(reflection.score).toBeLessThan(70); expect(reflection.feedback).toContain("不合格"); });
  });

  describe("6. RuntimeMetrics", () => {
    it("aggregates execution metrics", () => { const metrics = RuntimeMetrics.getInstance(); metrics.clear(); metrics.record({ agentId: "AG-1", missionId: "MS-1", toolName: "FileTool", promptLength: 250, inputTokens: 80, outputTokens: 40, latencyMs: 120, success: true }); const summary = metrics.getSummary(); expect(summary.totalCalls).toBe(1); expect(summary.totalTokens).toBe(120); expect(summary.byAgent["AG-1"].calls).toBe(1); expect(summary.byTool["FileTool"].calls).toBe(1); });
  });
