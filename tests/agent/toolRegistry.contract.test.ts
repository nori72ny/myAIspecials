...
describe('Agent tool registry security contract', () => {
  const expectedTools: ToolName[] = ['code_interpreter', 'document_generator', 'web_search_grounding', 'image_prompt_compiler', 'repository_explorer', 'file_reader', 'file_writer', 'verification_runner'];
  ...
  it('blocks execution without approval, failed safety policy, or non-zero cost', async () => {
    ...
    await expect(executeToolWithPermission('file_reader', { path: 'README.md' }, { approved: true, safetyPolicyPassed: false, costInUSD: 0 })).rejects.toThrow('SAFETY_POLICY_BLOCKED');
