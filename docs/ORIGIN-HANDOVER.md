...
At the time this handover was written, PR #124 is open, draft, unmerged, and must NOT be promoted to production merely because CI is green. Always re-fetch the current PR and HEAD SHA before acting.
...
- `src/agent/agentOrchestrator.ts`: HTTP/SSE orchestration, task execution, approval flow, verification/self-healing/checkpoint routes.
- `src/agent/toolRegistry.ts`: central tool contract and execution boundary. Eight registered tools currently include code_interpreter, document_generator, web_search_grounding, image_prompt_compiler, repository_explorer, file_reader, file_writer, verification_runner.
...
- `src/agent/checkpointManager.ts`: checkpoint history exists, but rollback is currently logical/representational rather than a true repository file restore. This is an outstanding issue.
- `src/agent/benchmarkRegressionGate.ts`: benchmark regression threshold gate.
- `services/mission-engine/src/application/agent/ToolExecutor.ts`: legacy engine boundary; legacy FileTool write is disabled, but legacy WebTool remains network-capable subject to its own URL/SSRF controls. Determine whether it is externally reachable before changing it.
- `services/mission-engine/src/application/agent/security/SafetyPolicyEngine.ts`: returns BLOCK/REVIEW. AgentRuntime blocks BLOCK for tool inputs; MessageValidator blocks both REVIEW and BLOCK for messages. Decide explicitly whether tool-input REVIEW should require human approval before future expansion.
...
### Race safety
The important property is not merely `realpath()` validation. Reads/writes must remain tied to stable opened directory/file descriptors so a symlink can not be swapped between validation and mutation.

### Writes
Use temp-file + atomic rename. Do not directly overwrite target files after validation. Keep the 256 KiB writer/editor budget aligned.
...
### Secrets
Secret-like content must be rejected before mutation. Never print secret contents in errors/logs.

### Network / shell
Default agent policy denies network and shell capabilities. Do not add unrestricted shell execution. If network is later enabled, require an explicit capability, strict allowlist, SSRF defenses, zero-cost proof where relevant, bounded response sizes/timeouts, and redaction.
...
1. **Real checkpoint rollback**: current rollback does not restore repository bytes. Implement true, atomic, verified restore using the same safe reader/writer boundary, or rename the API so it cannot be mistaken for a restore.
2. **Approval authenticity**: `approval.approved` / `intentExplicit` are request-level claims, not strong user authentication. If ORIGIN is remotely exposed, introduce authenticated sessions and server-side approval state/nonces before treating a write approval as authoritative.
3. **Legacy WebTool reachability**: determine whether the legacy mission-engine WebTool can be reached from a public route. If yes, reconcile it with the strict Phase 2 no-network boundary or isolate it behind an explicit capability.
4. **Safety REVIEW semantics**: determine whether tool-input REVIEW must pause for human approval. If yes, enforce it in AgentRuntime rather than only in messaging.
5. **Verification direct-exec portability**: current hardening is intentionally strict. Preserve fail-closed behavior on unsupported platforms; do not silently fall back to unsafe path traversal.
6. **Capability semantics audit**: every tool's declared capability and sideEffects must match its real behavior. Document generation should be modeled as in-memory artifact generation, not repository write.
7. **Benchmark quality**: current scoring integrity is improved, but future benchmark suites must measure real task outcomes, not only internal self-consistency. Add blind external baselines for coding/research/artifact tasks later.
