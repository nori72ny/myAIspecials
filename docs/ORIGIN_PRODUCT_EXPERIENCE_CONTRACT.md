...
Release strategy: Personal first, world-class agent service later

Last aligned: 2026-07-24

## 1. ORIGINが目指すもの

ORIGINは、単一のAIへ文章を送るチャット製品でも、複数のAIを並べて利用者に選ばせるツールでもない。ユーザーが一度目的を伝えると、表面的な依頼だけでなく達成に必要な情報、データ、判断材料、次の行動、成果物を見極め、必要な場合だけ適切な専門AIへ処理を委ね、検証可能な最終回答または成果物へ統合するAIエージェントサービスである。
...
一次目標はノリさんが日常的に安心して使えるPersonal Editionである。最終目標は、スマートフォン、タブレット、PCのすべてで、世界最高水準のUI、UX、情報設計、回答品質、対応範囲を持つAIエージェントサービスへ到達することである。「世界最高水準」は画面上の宣伝文句ではなく、比較可能な品質指標と利用者評価によって継続的に検証する目標として扱う。

ORIGINが提供する価値は、入力された依頼へ狭く答えることではない。依頼の目的達成に必要で、利用者がまだ明示していない情報やデータを過不足なく補い、重要な前提、選択肢、リスク、次の行動を提示する。ただし、不要な情報量を増やすことを「依頼以上の価値」と誤認せず、目的達成への寄与を基準に追加内容を選ぶ。

利用者や時期によって依頼内容は変わるため、法律、医療、開発など特定分野の固定一覧をORIGINの製品境界にしない。依頼を、必要能力、求める成果物、実行形式へ分けて判断する。現在想定する成果物には、資料、スライド、提案書、トークスクリプト、画像、アプリ、検索結果、Instagram投稿、Webサイト、表計算、グラフを含むが、これは上限ではない。

実行形式は、会話による回答、単一成果物の制作、複数工程を最後まで進めるエージェント実行を区別する。新しいAI、動画・音声・制作・検索サービスが登場した場合は、provider adapterと能力・成果物カタログを追加し、ORIGINの中核や一般UIを作り直さず対応する。

「世界最高水準」は、正確性、依頼と目的への適合、完成度、実用性、検証可能性、デザイン、全端末品質を成果物別に比較し、ノリさんによる受入評価を含めて証明する。比較証拠なしに「世界一」「最高精度」と表示しない。

## 2. 1つの依頼に対する標準フロー

1. 表面的な文言だけでなく、達成したい目的を理解する。
2. 依頼に明記されていなくても、目的達成に必要な情報、データ、前提、リスクを特定する。
3. 依頼の種類、重要度、最新情報の必要性、専門性、成果物形式を分類する。
4. 能力、無料状態、安全条件、利用可能性から実行候補を選ぶ。
5. 単独AIで十分か、専門AI、複数AI比較、独立確認が必要か判断する。
6. 必要な調査、根拠、引用、計算、分析、制作を行う。
7. 事実、論理、安全性、指示適合性、実行経路を検証する。
8. 複数の結果を、矛盾や少数意見を隠さず最終回答へ統合する。
9. 結論、追加価値、データ、成果物、次の行動を理解しやすい順序で提示する。
10. 一般利用者には処理結果を優先し、技術情報は段階的に開示する。
11. 保存・学習機能が正式実装されるまでは、利用者の記憶として保存したと表示しない。
...
- Dark Firstを維持しながら、ライト表示、スマートフォン、タブレット、PC、縦横回転、キーボード、タッチ、スクリーンリーダーで同等に使えるようにする。
- モバイルを縮小版、PCを引き伸ばしたモバイル版にしない。端末幅に応じて情報密度、ナビゲーション、回答幅、入力操作を最適化しながら、機能と信頼情報の意味を一致させる。
...
- ORIGINを複数AIの選択画面や実行数の可視化だけを価値とする製品へ縮小しない。
- スマートフォンだけを基準にせず、タブレットとPCを含む全対象viewportで主要操作を検証する。
