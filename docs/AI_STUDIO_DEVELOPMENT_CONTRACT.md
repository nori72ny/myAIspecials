...
ORIGINの製品目的、複数AI連携、回答体験、デザイン原則は`docs/ORIGIN_PRODUCT_EXPERIENCE_CONTRACT.md`を正とする。AI Studioは正式な開発ワークベンチであり、将来利用し得るprovider adapterの一つだが、ORIGIN全体をGemini専用チャットへ縮小しない。
...
- Nodeサーバーとserverless APIの構成を`createOriginApp`へ集約
- legacy/provider guardを全公式Express entrypointの先頭へ固定
- 旧Gemini、旧provider、Mission mutation経路の統合負テストを追加
- Gemini用環境変数が存在しても正式チャットがfallbackしないことを確認
- workerはprovider実行無効のまま維持

### Phase AS-2 — 実装済み・未接続

- `OriginAiStudioRuntimePolicy`を追加
- feature flagは既定OFF
- 本番用無料モデルカタログは意図的に空
- オーナーの実リクエスト承認がない場合は停止
- サーバー側認証情報の設定確認がない場合は停止
- Free Tier、入力・出力$0.00、公式根拠、期限内証拠を必須化
- preview、experimental、`latest` aliasを拒否
- Interactions API計画は`store=false`
- 自動再試行、自動provider fallback、自動モデル選択を禁止
- 実際のGemini HTTP clientや正式`/api/chat`には未接続

### Phase AS-3 — offline contract実装済み・実通信未接続

- 認証情報やHTTP headerを受け取らないoffline transport contractを追加
- rate limit、地域対象外、timeout、upstream拒否、不正応答を固定された安全なエラーへ変換
- timeout時は1回の処理を中止し、自動再試行しない
- provider fallbackと自動モデル選択を実行契約でも拒否
- prompt、Authorization、credential、upstream本文を安全ログへ渡さない負のテストを追加
- 無料・安全ポリシーが弱められた場合はtransport呼び出し前に停止
- 実Gemini HTTP adapter、APIキー参照、正式`/api/chat`接続は未実装

Phase AS-4へ進む前に、exact candidate SHAのCI全成功と、正式アカウントでのFree Tier根拠確認が必要。実リクエストはノリさんの別途承認があるまで行わない。

### Phase AS-3.2 — Interactions HTTP adapter実装済み・正式経路未接続

- Google公式仕様を2026-07-23に再確認
- 固定endpoint `POST /v1beta/interactions`だけを使用
- 認証情報は専用のサーバー環境変数`ORIGIN_AI_STUDIO_API_KEY`だけを参照
- APIキーをURL、本文、ログ、結果へ含めない
- request bodyを`model`、`input`、`store:false`だけに限定
- background、stream、tools、previous interaction、service tierを指定しない
- redirectを拒否し、1回の`fetch`以外に再試行しない
- 429、451、その他の失敗を固定された結果へ変換し、provider error bodyを読まない
- 成功応答は`completed`かつ`model_output`のtextだけを許可
- SDK専用の`output_text`、不完全・巨大・不正JSON応答を拒否
- adapterは正式`/api/chat`、runtime composition、UIのいずれにも未接続

Google公式資料では、2026年7月時点で新規キーはauthorization keyが既定で、Standard keyは2026年9月に拒否予定とされている。ORIGINはキー形式、アカウント、Cloud projectを自動変更せず、正式アカウント確認時に改めて公式仕様を確認する。

### Phase AS-3.3 — runtime coordinator実装済み・製品route未接続

- runtime flag、専用server認証情報、Free Tier証拠を承認前にpreflight
- preflight失敗時は承認gateを呼ばず、transportも実行しない
- 承認gateは既定で常に拒否
- 環境変数名に`APPROVED`が含まれていても承認として扱わない
- 承認gateの例外・不正値はfail closed
- 注入された承認もcoordinator内で1回だけ消費し、再利用を拒否
- 承認後も同じ時刻でplanと証拠期限を再検証
- 本番catalogは空のままなので、既定構成では承認gateへ到達しない
- coordinatorは`createOriginApp`、`/api/chat`、UI、workerへ未接続

実リクエストの承認を生成・保存・受領するproduction経路は意図的に実装していない。正式アカウント確認とノリさんの別途承認までは、テスト用に注入したsynthetic approvalとfake transport以外の実行を行わない。

### Phase AS-3.4 — official route isolation固定済み

- `/api/ai-studio`、`/api/interactions`、`/api/gemini`を先頭provider guardで全method停止
- Nodeとserverlessの正式`/api/chat`へAI Studio用runtime flagと専用認証情報を設定しても、既存の正式チャット境界を変更しない
- `ORIGIN_AI_STUDIO_OWNER_APPROVED=true`のような承認らしい環境変数を設定しても実行しない
- legacy・Mission経路は入力を解析・反映・送信する前に停止
- Cloudflare workerへ同じ環境変数を渡してもprovider executionは無効
- Node、serverless、workerの統合テストでfetch 0回を確認

この段階でも、AI Studio coordinatorを正式経路へ接続する変更は行っていない。将来接続する場合は、provider固有の新規公開endpointではなく、正式`/api/chat`境界・秘密情報遮断・Free Tier確認・単発承認を一体として再設計する。

### Phase AS-3.5 — 公開資料による候補モデル固定・有効化禁止

2026-07-23時点のGoogle公式公開資料から、`gemini-3.5-flash-lite`を将来確認する候補として記録した。本番用無料モデルカタログへは追加しておらず、runtime、route、UI、workerのいずれにも接続していない。

| 主張 | 証拠レベル | 現在の判断 |
|---|---|---|
| 明示モデルIDはstable/GAでInteractions API対応 | DOC-PROVEN | 候補条件を満たす |
| 公開価格表のFree Tier入力・出力料金 | DOC-PROVEN | ともに`$0.00` |
| `store:false`でinteraction objectを保存しない | DOC-PROVEN / CODE-PROVEN | adapterの固定契約あり |
| 正式アカウントがFree Tier | ACCOUNT-UNVERIFIED | 有効化blocker |
| 正式アカウントで候補モデルを利用可能 | ACCOUNT-UNVERIFIED | 有効化blocker |
| 正式projectのrate limit | ACCOUNT-UNVERIFIED | AI Studio上の確認が必要 |
| 実行環境の地域適合 | ACCOUNT-UNVERIFIED | 日本は公開対象地域だが実環境未確認 |
| 実リクエスト | UNVERIFIED | 未承認・未実行 |

重要なプライバシー差分として、公開価格表はFree TierのデータがGoogle製品の改善に使用されると示している。`store:false`はinteraction objectの保存を無効にする設定であり、製品改善へのデータ利用除外を証明しない。このため、利用者向け説明と受入方針が確定するまでは候補を有効化しない。

候補コードは実行用`OriginAiStudioFreeModelEvidence`と互換にせず、次の全blockerを明示している。

- 正式アカウントのtier未確認
- 正式アカウントでのモデル利用可否未確認
- 正式projectのrate limit未確認
- 実行環境の地域未確認
- Free Tierデータ利用説明の未受入
- 実リクエスト未承認

公開資料の確認期限は7日間に限定する。期限内であってもblockerは自動解除せず、本番catalogは空のまま維持する。
