...
self.addEventListener('install', (event) => {
  event.waitUntil(caches.open(CACHE_NAME).then(async (cache) => {
    ...
    } catch { /* offline-first installation fallback */ }
  }));
});
...
self.addEventListener('fetch', (event) => {
  ...
  if (url.pathname === '/origin-artifact-sandbox.html') { event.respondWith(caches.match(request).then((cached) => cached || fetch(request))); return; }
  if (request.mode === 'navigate') {
    event.respondWith(fetch(request).then(async (response) => {
      if (response.ok && !hasSensitiveHeaders) { const cache = await caches.open(CACHE_NAME); await cache.put(APP_SHELL_KEY, response.clone()); }
      return response;
    }).catch(async () => (!hasSensitiveHeaders && await caches.match(APP_SHELL_KEY)) || caches.match('/offline.html')));
    return;
  }
  if (hasSensitiveHeaders) return;
